...
function participantRole(booking: IBooking, userId: string): "USER" | "CREATOR" | null {
  if (String(booking.userId) === userId) return "USER";
  if (String(booking.creatorId) === userId) return "CREATOR";
  return null;
}

function disputeIneligibility(booking: IBooking): string | null {
  if (!["COMPLETED", "CANCELLED", "EXPIRED"].includes(booking.status)) return "Dispute not allowed for this booking status";
  if (booking.status !== "COMPLETED") return null;
  if (!booking.completedAt) return "Invalid booking completion state";
  if (Date.now() - booking.completedAt.getTime() > DISPUTE_WINDOW_MS) return "Dispute window expired (24 hours)";
  return null;
}

function disputeDto(dispute: IDispute, actorId: string, role: "USER" | "CREATOR") {
...
async function findParticipantBooking(bookingId: string, userId: string) {
  if (!mongoose.Types.ObjectId.isValid(bookingId)) return { booking: null, role: null };
...
export const openDispute = async (req: AuthenticatedRequest, res: Response) => {
  const user = req.user;
  const { bookingId, reason } = req.body as { bookingId?: unknown; reason?: unknown };
  if (!user) return res.status(401).json({ message: "Unauthorized" });
  if (typeof bookingId !== "string" || typeof reason !== "string") return res.status(400).json({ message: "bookingId and reason are required" });
  const trimmedReason = reason.trim();
  if (!trimmedReason) return res.status(400).json({ message: "reason is required" });
  if (trimmedReason.length > 1000) return res.status(400).json({ message: "reason must be at most 1000 characters" });

  const { booking, role } = await findParticipantBooking(bookingId, user.id);
  if (!booking) return res.status(mongoose.Types.ObjectId.isValid(bookingId) ? 404 : 400).json({ message: mongoose.Types.ObjectId.isValid(bookingId) ? "Booking not found" : "Invalid bookingId" });
  if (!role) return res.status(403).json({ message: "Access denied" });
  const ineligibilityReason = disputeIneligibility(booking);
  if (ineligibilityReason) return res.status(400).json({ message: ineligibilityReason });

  if (await Dispute.exists({ bookingId: booking._id })) return res.status(409).json({ message: "A dispute already exists for this booking" });
  try {
    const dispute = await Dispute.create({ bookingId: booking._id, raisedBy: user.id, raisedByRole: role, reason: trimmedReason, status: "OPEN", customerInput: { state: "OPEN" }, creatorInput: { state: "OPEN" } });
    return res.status(201).json({ message: "Dispute opened successfully", dispute: disputeDto(dispute, user.id, role) });
  } catch (error) {
    if (isDuplicateKeyError(error)) return res.status(409).json({ message: "A dispute already exists for this booking" });
    throw error;
  }
};

export const getBookingDisputeState = async (req: AuthenticatedRequest, res: Response) => {
  const user = req.user;
  if (!user) return res.status(401).json({ message: "Unauthorized" });
  const { booking, role } = await findParticipantBooking(req.params.bookingId, user.id);
  if (!booking) return res.status(mongoose.Types.ObjectId.isValid(req.params.bookingId) ? 404 : 400).json({ message: mongoose.Types.ObjectId.isValid(req.params.bookingId) ? "Booking not found" : "Invalid bookingId" });
  if (!role) return res.status(403).json({ message: "Access denied" });
