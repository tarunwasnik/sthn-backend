...
const all = (source: string, markers: readonly string[]) =>
...
const documentation = fs.readFileSync(
  path.join(workspace, "docs/implementation/phase-7g-top-up-reconciliation-recovery.md"),
  "utf8",
);
...
expect(failure.includes("PROVIDER_FAILURE_CONFLICT"),
  "Provider failure conflict is not bounded.");
...
expect(!repair.includes("req.body") && !repair.includes("$set: req") &&
  !controller.includes("fieldValue") && !controller.includes("patch"),
  "Repair accepts an arbitrary field patch.");
expect(all(requestRepository, [
...
expect(all(audit, ["actorType === \"ADMIN\"", "actorId", "reasonCode", "createdAt"]),
  "Operational audit identity or safe result recording is incomplete.");
...
expect(!controller.includes("req.body.admin") && !controller.includes("adminUserId: req.body"),
  "Admin identity is accepted from the request body.");
expect(!controller.includes("WalletTopUpRequest.find") &&
  !controller.includes("WalletTopUpReconciliation.find"),
  "Admin controller accesses persistence directly.");
expect(all(routes, [
...
expect(packageJson.includes('"validate:phase7g": "node -r ts-node/register src/scripts/validatePhase7G.ts"'),
  "validate:phase7g package command is missing.");
