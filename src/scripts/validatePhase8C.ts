...
for (const file of required) expect(exists(file), `Missing Phase 8C file: ${file}`);
...
expect(!/Wallet\.(update|findOneAndUpdate|findByIdAndUpdate)/.test(service),
  "Capture service directly mutates Wallet.");
expect(projectionService.includes("applyConditionalDelta"),
  "Wallet projection does not use an atomic conditional update.");
...
expect(!/Date\.now|new Date|randomUUID|Math\.random/.test(identity),
  "Capture identity contains mutable or random input.");
...
expect(controller.includes("BookingWalletReservationCaptureError") &&
  ...
  "Capture controller error mapping missing.");
...
expect(!service.includes("WALLET_CAPTURED"),
  "An unauthorized customer Wallet captured bucket was introduced.");
expect(completion.includes("payment.method === PaymentMethod.WALLET") &&
  completion.includes("payment.method === PaymentMethod.INTERNAL"),
  "Provider and Wallet booking branches are not explicit.");
expect(packageJson.includes('"validate:phase8c"') && packageJson.includes('"test:phase8c"'),
  "Phase 8C package commands missing.");
expect(
  fs.existsSync(path.join(workspace, "docs/implementation/phase-8c-booking-wallet-reservation-capture.md")),
  "Phase 8C documentation missing.",
);

console.log(
  "Phase 8C Booking Wallet reservation capture static validation passed; MongoDB behavior requires test:phase8c.",
);
