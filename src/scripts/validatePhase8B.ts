...
for (const file of required) expect(exists(file), `Missing Phase 8B file: ${file}`);
...
expect(!/Wallet\.(update|findOneAndUpdate|findByIdAndUpdate)/.test(service),
  "Release service directly mutates Wallet.");
expect(projectionService.includes("applyConditionalDelta"),
  "Wallet projection does not use a database-atomic conditional update.");
...
expect(!/Date\.now|new Date|randomUUID|Math\.random/.test(identity),
  "Release identity contains mutable or random input.");
...
expect(expiry.includes("expireBookingsJob") && expiry.includes("BOOKING_EXPIRED"),
  "Expiry integration missing.");
expect(userCancellation.includes("terminateBookingFinancially"),
  "User cancellation integration missing.");
expect(creatorCancellation.includes("terminateBookingFinancially"),
  "Creator cancellation integration missing.");
...
expect(tests.includes("BookingFundReservationStatus.ACTIVE") &&
  ...
  tests.includes("Creator ACCEPT keeps"),
  "ACCEPT regression proof missing.");
...
expect(termination.includes("payment?.method === PaymentMethod.WALLET"),
  "Provider and Wallet booking branches are not explicit.");
expect(packageJson.includes('"validate:phase8b"') && packageJson.includes('"test:phase8b"'),
  "Phase 8B package commands missing.");
expect(
  fs.existsSync(path.join(workspace, "docs/implementation/phase-8b-booking-wallet-reservation-release.md")),
  "Phase 8B documentation missing.",
);
