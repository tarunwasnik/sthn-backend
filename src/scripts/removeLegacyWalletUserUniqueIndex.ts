...
export interface WalletIndexDescription {
  name?: string;
  key: Record<string, unknown>;
  unique?: boolean;
}

export interface LegacyWalletIndexPlan {
  staleUserUniqueIndex?: WalletIndexDescription;
  compoundCurrencyIndex: WalletIndexDescription;
}

const keysEqual = (actual: Record<string, unknown>,
...
export const planLegacyWalletUserUniqueIndexRemoval = (
  indexes: WalletIndexDescription[],
): LegacyWalletIndexPlan => {
  const compoundCurrencyIndex = indexes.find((index) =>
    index.unique === true && keysEqual(index.key, { userId: 1, currency: 1 }));
  ...
  if (!namedUserIndex) return { compoundCurrencyIndex };
...
const hasDuplicateWalletOwnershipCurrencies = async (): Promise<number> => {
...
const main = async () => {
  ...
  try {
    const indexes = await Wallet.collection.indexes() as WalletIndexDescription[];
    const plan = planLegacyWalletUserUniqueIndexRemoval(indexes);
    ...
    await Wallet.collection.dropIndex(plan.staleUserUniqueIndex.name!);
