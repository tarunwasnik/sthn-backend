...
for (const file of required) expect(exists(file), `Missing Phase 9D file: ${file}`);
...
hasAll(status, ['COMPLETED = "COMPLETED"', 'FAILED = "FAILED"'],
  "Withdrawal terminal statuses incomplete");
expect(!status.includes("PROCESSING") && !status.includes("CANCELLED"),
  "Phase 9D introduced a forbidden withdrawal status.");
hasAll(account, ["WITHDRAWAL_RESERVED", "WALLET_AVAILABLE", "PAYOUT_CLEARING"],
  "Required Ledger accounts missing");
hasAll(entryType, ["CREATOR_WITHDRAWAL_COMPLETED", "CREATOR_WITHDRAWAL_FAILED_RELEASED"],
  "Finalization Ledger types missing");
...
"Finalization model authority incomplete");
...
hasAll(identity, ["finalizationFingerprint", "finalizationKey",
  ...
  "providerTerminalStatus", "reservationTransactionId", "outcome"],
"Deterministic finalization identity incomplete");
hasAll(service, ["withdrawalProviderExecutionService.validateReplay",
  ...
  "createFinancialAudit", "AFTER_FINALIZATION_IDENTITY", "BEFORE_COMMIT"],
"Finalization orchestration incomplete");
...
hasAll(error, ["WITHDRAWAL_NOT_FOUND", "PROVIDER_NOT_FOUND",
  ...
  "OUTCOME_CONFLICT", "REPLAY_CONFLICT", "INTEGRITY_ERROR"],
"Finalization error contract incomplete");
hasAll(tests, ["successful provider reservation exactly once",
  ...
  "does not execute providers or touch legacy financial domains"],
"Phase 9D runtime proof incomplete");
expect(packageJson.includes('"validate:phase9d"') && packageJson.includes('"test:phase9d"'),
  "Phase 9D package commands missing.");
expect(fs.existsSync(path.join(workspace,
  "docs/implementation/phase-9d-withdrawal-finalization.md")),
"Phase 9D documentation missing.");
