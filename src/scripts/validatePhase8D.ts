...
for (const file of required) expect(exists(file), `Missing Phase 8D file: ${file}`);
...
expect(!service.includes("walletProjectionService"),
  "Allocation invokes WalletProjectionService.");
expect(!/Wallet\.(update|findOneAndUpdate|findByIdAndUpdate|create)/.test(service),
  "Allocation directly mutates a Wallet.");
expect(!service.includes("walletId:"),
  "Allocation creates a Wallet-linked posting.");
...
expect(!/Date\.now|new Date|randomUUID|Math\.random/.test(identity),
  "Allocation identity contains mutable or random input.");
...
for (const code of [
  ...
  "FINANCIAL_LOCKED",
  "INTEGRITY_ERROR",
]) expect(errors.includes(`BOOKING_ESCROW_ALLOCATION_${code}`),
  `Allocation error code missing: ${code}`);
...
expect(packageJson.includes('"validate:phase8d"') &&
  packageJson.includes('"test:phase8d"'),
  "Phase 8D package commands missing.");
expect(
  fs.existsSync(path.join(
    workspace,
    "docs/implementation/phase-8d-captured-escrow-allocation.md",
  )),
  "Phase 8D documentation missing.",
);

console.log(
  "Phase 8D captured escrow allocation static validation passed; MongoDB behavior requires test:phase8d.",
);
