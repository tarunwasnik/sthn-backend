...
hasAll(status, ['CREATED = "CREATED"', 'INITIALIZED = "INITIALIZED"'],
  "Phase 9B provider statuses missing");
for (const forbidden of [
  ...
  expect(!service.includes(forbidden),
    `Phase 9B implements later provider execution: ${forbidden}`);
}
hasAll(model, [
...
for (const forbidden of [
  ...
  expect(!service.includes(forbidden),
    `Phase 9B contains forbidden side effect or execution: ${forbidden}`);
}
expect(!/Wallet\.(update|findOneAndUpdate|findByIdAndUpdate|create)/.test(service),
  "Phase 9B service mutates Wallet.");
expect(!/LedgerEntry\.(create|update|findOneAndUpdate)/.test(service),
  "Phase 9B service mutates Ledger.");
hasAll(tests, [
...
expect(packageJson.includes('"validate:phase9b"') &&
  packageJson.includes('"test:phase9b"'),
  "Phase 9B package commands missing.");
expect(fs.existsSync(path.join(
  workspace,
  "docs/implementation/phase-9b-withdrawal-provider-initialization.md",
)), "Phase 9B documentation missing.");

console.log(
  "Phase 9B withdrawal provider initialization static validation passed; MongoDB behavior requires test:phase9b.",
);
