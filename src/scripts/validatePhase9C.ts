...
const withdrawalRepository = read(
  "src/repositories/creatorWithdrawalRequest.repository.ts",
);
const simulator = read(
  "src/services/providerSimulator/providerSimulator.service.ts",
);
const identity = read(
  "src/utils/financial/withdrawalProviderIdentity.util.ts",
);
const eventTypes = read(
  "src/constants/internalProvider/providerEventType.enum.ts",
);
...
expect(!status.includes("CANCELLED"),
  "Phase 9C introduced forbidden cancellation status.");
...
for (const forbidden of [
  ...
  expect(!service.includes(forbidden),
    `Phase 9C contains forbidden accounting/finalization: ${forbidden}`);
}
hasAll(tests, [
...
expect(packageJson.includes('"validate:phase9c"') &&
  packageJson.includes('"test:phase9c"'),
  "Phase 9C package commands missing.");
expect(fs.existsSync(path.join(
  workspace,
  "docs/implementation/phase-9c-withdrawal-provider-execution.md",
)), "Phase 9C documentation missing.");

console.log(
  "Phase 9C withdrawal provider execution static validation passed; MongoDB behavior requires test:phase9c.",
);
