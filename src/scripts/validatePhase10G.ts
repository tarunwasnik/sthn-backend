...
const decision = read("enums", "financial",
  "walletConversionDecision.enum.ts");
const rejection = read("enums", "financial",
  "walletConversionRejectionCode.enum.ts");
...
const auditRepository = read("repositories",
  "walletConversionAudit.repository.ts");
const service = read("services", "financial",
  "adminWalletConversionDecision.service.ts");
...
const controller = read("controllers",
  "adminWalletConversionDecision.controller.ts");
...
const testRoot = path.join(src, "tests", "financial", "phase10g");
...
expect(all(status, ['PENDING = "PENDING"', 'APPROVED = "APPROVED"',
  'REJECTED = "REJECTED"']) &&
  !/PROCESSING|CANCELLED|EXPIRED/.test(status),
"Phase 10G lifecycle is incomplete or includes an invalid state.");
expect(all(decision, ['APPROVE = "APPROVE"', 'REJECT = "REJECT"']) &&
  (decision.match(/=\s*"/g) ?? []).length === 2,
"Wallet conversion decision enum is not bounded.");
expect(all(rejection, ["ADMIN_DECLINED", "INVALID_REQUEST",
  "FX_SNAPSHOT_NOT_ACCEPTABLE", "INSUFFICIENT_SOURCE_FUNDS",
  "SIMULATION_REJECTED", "OTHER"]),
"Wallet conversion rejection-code enum is incomplete.");
expect(all(repository, ["approvePending", "rejectPending",
  ...
  "WalletConversionRequestStatus.REJECTED", "findOneAndUpdate"]),
"Guarded PENDING decision transitions are incomplete.");
expect(!/genericUpdate|updateStatus|setStatus|deleteRequest|removeRequest/.test(
  repository), "A generic request mutation method exists.");
...
expect(all(model, ["conversionReference", "conversionKey", "userId",
  ...
  "rejectionCode", "rejectionReason"]),
"Immutable request identity or narrow decision metadata is incomplete.");
...
"Approval snapshot eligibility or balance precheck is missing.");
expect(all(requestService, ["validateStoredAuthority",
  "requireStoredSnapshotEligible", "validateStoredSnapshot",
  "availableBalance < amount", "targetWalletId"]),
"Stored request, Wallet, or snapshot validation is incomplete.");
expect(all(fxService, ["requireStoredSnapshotEligible",
  "validateStoredSnapshot", "INVALIDATED", "expiresAt"]),
"Phase 10E does not remain the stored FX snapshot authority.");
expect(!/lookupOrRefresh|forceRefresh|\.refresh\(|getReferenceRate/.test(service),
  "Admin decision invokes or refreshes the FX provider.");
expect(!/reserve|applyConditionalDelta|createWallet|Wallet\.create/.test(service),
  "Admin decision reserves funds or creates/mutates a Wallet.");
expect(all(service, ["terminal", "DECISION_CONFLICT", "decidedBy",
  "decidedAt", "rejectionReason", "findByAuditKey"]),
"Exact terminal replay and audit-authority validation are incomplete.");
expect(all(auditAction, ["WALLET_CONVERSION_APPROVED",
  ...
  all(auditRepository, ["createOnce", "findByAuditKey"]),
"Decision audit authority is incomplete.");
expect(all(service, ["withTransaction", "approvePending", "rejectPending",
  "createOnce", "session"]),
"Decision and required audit are not transactionally coupled.");
expect(all(routes, ["router.use(protect, authorizeRoles(\"admin\"))",
  "/wallet-conversion-requests", ":conversionReference", "/decision"]),
"Admin-only decision/list/detail routes are incomplete.");
...
"Safe User/Admin decision DTO is incomplete or leaks internal identity.");
expect(all(tests, ["ten approvals", "ten identical rejections",
  ...
  "AFTER_GUARDED_TRANSITION", "AFTER_AUDIT", "BEFORE_COMMIT"]),
"Phase 10G runtime proof coverage is incomplete.");
expect(testFiles.some((file) => file.endsWith(
  "walletConversionDecisionApproval.test.ts")) &&
  testFiles.some((file) => file.endsWith(
    "walletConversionDecisionRejection.test.ts")) &&
  testFiles.some((file) => file.endsWith(
    "walletConversionDecisionReplay.test.ts")) &&
  testFiles.some((file) => file.endsWith(
    "walletConversionDecisionConcurrency.test.ts")) &&
  testFiles.some((file) => file.endsWith(
    "walletConversionDecisionFailure.test.ts")) &&
  testFiles.some((file) => file.endsWith(
    "walletConversionDecisionIntegrity.test.ts")) &&
  testFiles.some((file) => file.endsWith(
    "walletConversionDecisionRoutes.test.ts")) &&
  testFiles.some((file) => file.endsWith(
    "walletConversionDecisionRegression.test.ts")),
"Required Phase 10G runtime files are missing.");
expect(!new RegExp([
...
].join("|")).test(production),
"Phase 10G touches accounting or another financial domain.");
expect(!new RegExp([
...
expect(requestService.includes("createPending") &&
  repository.includes("status: WalletConversionRequestStatus.PENDING"),
"Phase 10F is no longer the request-creation authority.");
expect(fxService.includes("validatePersisted") &&
  fxService.includes("snapshotFingerprint"),
"Phase 10E is no longer the FX snapshot-integrity authority.");
expect(packageJson.includes('"validate:phase10g"') &&
  ...
  packageJson.includes('"test:phase10g:integrity"'),
"Phase 10G package commands are missing.");
expect(fs.existsSync(path.join(workspace, "docs", "implementation",
  "phase-10g-wallet-conversion-admin-decision.md")),
"Phase 10G documentation is missing.");
