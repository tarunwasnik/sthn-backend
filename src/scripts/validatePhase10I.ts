...
const identity = read("utils", "financial",
  "walletConversionAccountingIdentity.util.ts");
const service = read("services", "financial",
  "walletConversionAccounting.service.ts");
const dto = read("dtos", "wallet",
  "walletConversionAccounting.response.dto.ts");
const controller = read("controllers",
  "adminWalletConversionAccounting.controller.ts");
...
const testRoot = path.join(src, "tests", "financial", "phase10i");
...
expect(all(status, ['APPROVED = "APPROVED"', 'COMPLETED = "COMPLETED"',
  'FAILED = "FAILED"']) && !/PROCESSING|CANCELLED/.test(status),
"Phase 10I request terminal lifecycle is not bounded.");
expect(ledgerType.includes("WALLET_CONVERSION_COMPLETED") &&
  ledgerSource.includes("WALLET_CONVERSION"),
"Conversion-specific Ledger type or source is missing.");
...
"Wallet conversion accounting authority fields are incomplete.");
...
"Accounting repository transitions are not narrow and guarded.");
expect(all(identity, ["conversionReference", "conversionKey",
  ...
  "sourceProjectionKey", "targetProjectionKey"]),
"Deterministic conversion accounting identity is incomplete.");
expect(all(service, ["walletCreationService.createWallet", "ledgerService",
  ...
  "validateReplay", "withTransaction"]),
"Conversion Ledger, Wallet creation, projection, or replay flow is incomplete.");
...
"Required Phase 10I rollback boundaries are incomplete.");
expect(!/lookupOrRefresh|forceRefresh|\.refresh\(|simulateWalletConversionProvider|Wallet\.findOneAndUpdate|Wallet\.update/.test(service),
"Phase 10I refreshes FX, invokes the provider, or mutates Wallet directly.");
expect(all(auditAction, ["WALLET_CONVERSION_COMPLETED",
  ...
  all(service, ["WalletConversionAuditAction.COMPLETED",
    "WalletConversionAuditAction.FAILED", "createOnce"]),
"Phase 10I completion/failure audit is incomplete.");
expect(all(dto, ["conversionReference", "status", "sourceCurrency",
  "targetCurrency", "sourceAmount", "targetAmount", "completedAt"]) &&
  !/userId|walletId|projectionReference|transactionReference|fingerprint|key/.test(dto),
"Phase 10I accounting DTO is unsafe or incomplete.");
...
"Phase 10I Admin-only empty-body accounting route is incomplete.");
expect(all(tests, ["cross-currency Ledger and Wallet accounting",
  ...
  "Admin accounting route"]),
"Required Phase 10I runtime proof is incomplete.");
for (const file of ["phase10i.runtime.test.ts",
  ...
  expect(testFiles.some((candidate) => candidate.endsWith(file)),
    `Required Phase 10I runtime file is missing: ${file}`);
}
expect(packageJson.includes('"validate:phase10i"') &&
  packageJson.includes('"test:phase10i"'),
"Phase 10I package commands are missing.");
expect(fs.existsSync(path.join(workspace, "docs", "implementation",
  "phase-10i-wallet-conversion-accounting.md")),
"Phase 10I documentation is missing.");
