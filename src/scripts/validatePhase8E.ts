...
for (const file of required) expect(exists(file), `Missing Phase 8E file: ${file}`);
...
expect(!/Wallet\.(update|findOneAndUpdate|findByIdAndUpdate|create)/.test(service),
  "Settlement directly mutates a Wallet.");
expect(!service.includes("walletCreationService"),
  "Settlement automatically creates a Creator Wallet.");
expect(!service.includes("LedgerAccount.PLATFORM_ESCROW,"),
  "Settlement posts to PLATFORM_ESCROW.");
expect(!service.includes("LedgerAccount.PLATFORM_COMMISSION_PAYABLE,"),
  "Settlement posts to PLATFORM_COMMISSION_PAYABLE.");
...
expect(!/Date\.now|new Date|randomUUID|Math\.random/.test(identity),
  "Settlement identity contains mutable or random input.");
...
hasAll(accounts, ["CREATOR_PAYABLE", "WALLET_AVAILABLE"],
  "Settlement Ledger accounts missing");
hasAll(entryTypes, ["BOOKING_CREATOR_SETTLED"],
  "Settlement Ledger type missing");
hasAll(sources, ["BOOKING_CREATOR_WALLET_SETTLEMENT"],
  "Settlement Ledger source missing");
hasAll(auditActions, ["BOOKING_CREATOR_WALLET_SETTLED"],
  "Settlement audit action missing");
...
for (const code of [
  ...
  "COMPLETION_CONFLICT",
  "INTEGRITY_ERROR",
]) expect(errors.includes(`BOOKING_CREATOR_SETTLEMENT_${code}`),
  `Settlement error code missing: ${code}`);
...
for (const forbidden of [
  ...
  `Forbidden Phase 8E service dependency found: ${forbidden}`);

expect(packageJson.includes('"validate:phase8e"') &&
  packageJson.includes('"test:phase8e"'),
  "Phase 8E package commands missing.");
expect(
  fs.existsSync(path.join(
    workspace,
    "docs/implementation/phase-8e-creator-payable-wallet-settlement.md",
  )),
  "Phase 8E documentation missing.",
);

console.log(
  "Phase 8E Creator payable Wallet settlement static validation passed; MongoDB behavior requires test:phase8e.",
);
