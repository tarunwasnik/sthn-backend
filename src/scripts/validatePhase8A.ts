...
for (const file of requiredFiles) expect(exists(file), `Missing Phase 8A file ${file}.`);
...
expect(identity.includes('createHash("sha256")') &&
  ...
  !identity.includes("new Date"),
  "Reservation identity is not stable and deterministic.");
expect(service.includes("authenticatedUserId") &&
  ...
  service.includes("booking.currency !== currency"),
  "Authenticated ownership, server amount, or currency authority is incomplete.");
expect(service.includes("minimums: { availableBalance: booking.totalAmount }") &&
  ...
  service.includes("lockedBalance: 0"),
  "Atomic available-to-reserved projection is incomplete.");
expect(service.includes("ledgerService.createDebit") &&
  service.includes("ledgerService.createCredit") &&
  service.includes("walletProjectionService.applyProjectionMutation"),
  "Existing LedgerService and WalletProjectionService are not reused.");
...
expect(repository.includes("markActiveFromPending") &&
  ...
  !repository.includes("updateMany"),
  "Reservation lifecycle repository is not narrowly guarded.");
expect(controller.includes("bookingWalletReservationService.authorize") &&
  ...
  !controller.includes("walletProjectionService."),
  "Controller integration or Internal Provider regression boundary is incomplete.");
expect(controller.includes("safeWalletBookingResponse") &&
  !controller.match(/safeWalletBookingResponse[\s\S]{0,1500}ledgerEntryIds/),
  "Wallet booking response is not safely bounded.");
...
]) expect(!service.includes(forbidden) && !controller.includes(forbidden),
  `Forbidden process-local concurrency authority found: ${forbidden}.`);
...
]) expect(!service.includes(deferredImplementation),
  `Deferred Phase 8 behavior was implemented: ${deferredImplementation}.`);

const packageJson = read("package.json");
expect(packageJson.includes('"validate:phase8a"') && packageJson.includes('"test:phase8a"'),
  "Phase 8A package scripts are missing.");
expect(
  fs.existsSync(path.join(workspace, "docs/implementation/phase-8a-booking-wallet-reservation.md")),
  "Phase 8A documentation is missing.",
);
