...
import { deriveBookingRequestIdentity } from "../utils/financial/bookingWalletReservationIdentity.util";
...
const FINAL_BAN_GATE_COUNTERPARTY = "dispose@7.com" as const;

type DisposableEmail = (typeof DISPOSABLE_EMAILS)[number];
type G6SessionEmail = DisposableEmail | typeof FINAL_BAN_GATE_COUNTERPARTY;
type JsonRecord = Record<string, unknown>;
type G6BookingRead = { _id: mongoose.Types.ObjectId; userId: mongoose.Types.ObjectId; creatorId: mongoose.Types.ObjectId; slotIds: mongoose.Types.ObjectId[]; bookingReference?: string; status: string; totalAmount: number; currency: string };
...
const safeBackendErrorSummary = (value: unknown): string => {
  ...
  if (isRecord(value.error)) {
    ...
    for (const key of ["code", "message"]) {
      if (typeof value.error[key] === "string") nested[key] = value.error[key];
    }
    if (Object.keys(nested).length) summary.error = nested;
  ...
  if (Array.isArray(value.details)) {
    summary.details = value.details.slice(0, 5).map((detail) => {
      ...
      if (!isRecord(detail)) return "invalid detail";
...
const assertSafeRuntime = () => {
  ...
  if (process.env.NODE_ENV === "production") fail("NODE_ENV=production is not allowed");
...
function assertDisposable(email: string): asserts email is DisposableEmail {
  if (!(DISPOSABLE_EMAILS as readonly string[]).includes(email)) {
...
class G6AdminApi {
  ...
  static async login(baseUrl: string) {
    ...
    if (email !== ADMIN_EMAIL) fail("G6_ADMIN_EMAIL must equal the authorized Admin email");
    if (!password) fail("G6_ADMIN_PASSWORD is required");
    ...
    if (!response.ok || typeof token !== "string") throw new Error("G6 runtime safety check failed: Admin login failed");
  ...
  async patch(path: string, body: JsonRecord): Promise<unknown> {
    return this.request(path, { method: "PATCH", body: JSON.stringify(body) });
  }

  async previewGovernanceAction(input: { key: string; target: { email: string; id: string }; params: JsonRecord; reason: string }) {
  ...
  async executeConfirmedGovernanceAction(input: {
    key: string; target: { email: string; id: string }; params: JsonRecord; reason: string; confirmationToken: string;
  }) {
    assertDisposable(input.target.email);
    return this.request("/api/v1/admin/actions/execute", {
  ...
  private async request(path: string, init: RequestInit): Promise<unknown> {
    ...
    const body: unknown = await response.json().catch(() => null);
    if (!response.ok) fail(`Admin HTTP ${response.status} for ${init.method ?? "GET"} ${path}${safeBackendErrorSummary(body)}`);
    return body;
  }
}
...
const discoverWhitelistedAccounts = async () => {
  ...
  if (!mongoUri) throw new Error("G6 runtime safety check failed: MONGODB_URI is required only for read-only account discovery");
  await mongoose.connect(mongoUri, { serverSelectionTimeoutMS: 10_000 });
  try {
    const users = await User.find({ email: { $in: DISPOSABLE_EMAILS } })
    ...
    for (const email of DISPOSABLE_EMAILS) {
      ...
      const [profile, creator, wallets, customerBookings, creatorBookings] = await Promise.all([
        UserProfile.findOne({ userId: user._id }).select("profileStatus").lean(),
        CreatorProfile.findOne({ userId: user._id }).select("status slug").lean(),
        Wallet.find({ userId: user._id }).select("currency availableBalance reservedBalance lockedBalance currentBalance").lean(),
        Booking.find({ userId: user._id }).select("bookingReference status paymentStatus currency totalAmount").limit(20).lean(),
        Booking.find({ creatorId: user._id }).select("bookingReference status paymentStatus currency totalAmount").limit(20).lean(),
      ]);
...
const unwrapData = (value: unknown): unknown => isRecord(value) ? value.data : undefined;
...
const runDiscovery = async () => {
  ...
  if (missing.length) fail(`registry is missing required Governance actions: ${missing.join(", ")}`);
...
const runB2Inspection = async () => {
  const mongoUri = process.env.MONGODB_URI;
  if (!mongoUri) return fail("MONGODB_URI is required for the read-only B2 inspection");

  await mongoose.connect(mongoUri, { serverSelectionTimeoutMS: 15_000 });
  try {
    const booking = await Booking.findOne({ bookingReference: B2_BOOKING_REFERENCE })
    ...
    const [payment, reservation, slots, allocation, creatorSettlement, legacySettlements, dispute] = await Promise.all([
      Payment.findOne({ bookingId: b2._id })
        ...
        .lean(),
      BookingFundReservation.findOne({ bookingId: b2._id })
      ...
      BookingEscrowAllocation.findOne({ bookingId: b2._id })
        ...
        .lean(),
      BookingCreatorSettlement.findOne({ bookingId: b2._id })
...
const runEnableAdminActions = async () => {
  ...
  if (!Array.isArray(flags)) return fail("feature-flag list returned an invalid response");
  const flagList: unknown[] = flags;
  const existing = flagList.find((flag: unknown) => getString(flag, "key") === "ADMIN_ACTIONS_ENABLED");
  const payload = {
    enabled: true,
    scope: "ROLE",
  ...
  if (existing) {
    ...
    if (!flagId) fail("ADMIN_ACTIONS_ENABLED exists without a safe flag identifier");
  ...
  const verified = Array.isArray(verifiedFlags)
    ? verifiedFlags.find((flag: unknown) => getString(flag, "key") === "ADMIN_ACTIONS_ENABLED")
    : undefined;
  ...
  const mongoUri = process.env.MONGODB_URI;
  if (!mongoUri) return fail("MONGODB_URI is required for the read-only G6 dry-run target verification");
  await mongoose.connect(mongoUri, { serverSelectionTimeoutMS: 15_000 });
  try {
    const target = await User.findOne({ email: "dispose@1.com" })
      .select("email status governanceState")
      .lean();
    if (!target) return fail("dispose@1.com was not found for the G6 dry-run");
...
const PASSWORD_VARIABLES: Record<DisposableEmail, string> = {
...
class DisposableSession {
  private constructor(readonly email: G6SessionEmail, readonly role: string, private readonly baseUrl: string, private readonly token: string) {}
  static async login(baseUrl: string, email: G6SessionEmail, password: string) {
    const response = await fetch(`${baseUrl}/api/auth/login`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, password }), signal: AbortSignal.timeout(10_000) });
    const body: unknown = await response.json().catch(() => null);
    const token = getString(body, "token");
    if (!response.ok || typeof token !== "string") throw new Error(`G6 runtime safety check failed: login failed for ${email}`);
    ...
    if (getString(me, "email") !== email || typeof role !== "string") throw new Error(`G6 runtime safety check failed: identity verification failed for ${email}`);
    return new DisposableSession(email, role, baseUrl, token);
  }
  async get(path: string): Promise<unknown> { return this.request(path, { method: "GET" }); }
  async post(path: string, body: JsonRecord, headers: Record<string, string> = {}): Promise<unknown> { return this.request(path, { method: "POST", headers, body: JSON.stringify(body) }); }
  private async request(path: string, init: RequestInit): Promise<unknown> {
    const response = await fetch(`${this.baseUrl}${path}`, { ...init, headers: { Authorization: `Bearer ${this.token}`, "Content-Type": "application/json", ...(init.headers ?? {}) }, signal: AbortSignal.timeout(15_000) });
    const body: unknown = await response.json().catch(() => null);
    if (!response.ok) fail(`${this.email} HTTP ${response.status} for ${init.method ?? "GET"} ${path}${safeBackendErrorSummary(body)}`);
    return body;
  }
}

type ScenarioCurrency = "INR" | "USD" | "THB";
type Scenario = { label: string; customer: DisposableEmail; creator: DisposableEmail; currency: ScenarioCurrency; timing: "REQUESTED" | "LT24" | "GT24" };
const setupScenarios: Scenario[] = [
...
const scenarioPurpose = (label: string) => label.startsWith("CS") ? "Creator suspension and activation" : label.startsWith("CB") ? "Creator ban" : label.startsWith("CD") ? "Creator cooldown unchanged-booking baseline" : label.startsWith("B") ? "Customer ban" : "Customer suspension and activation";

const runSetup = async () => {
  const baseUrl = assertSafeRuntime();
  const missing = DISPOSABLE_EMAILS.filter((email) => !process.env[PASSWORD_VARIABLES[email]]);
  if (missing.length) fail(`missing required password variables: ${missing.map((email) => PASSWORD_VARIABLES[email]).join(", ")}`);
  const sessions = new Map<DisposableEmail, DisposableSession>();
  for (const email of DISPOSABLE_EMAILS) sessions.set(email, await DisposableSession.login(baseUrl, email, process.env[PASSWORD_VARIABLES[email]]!));
  const expectedRoles: Record<DisposableEmail, string> = { "dispose@1.com": "user", "dispose@2.com": "user", "dispose@3.com": "user", "dispose@4.com": "creator", "dispose@5.com": "creator", "dispose@6.com": "creator" };
  for (const [email, session] of sessions) { if (session.role !== expectedRoles[email]) fail(`unexpected role for ${email}`); console.log(`AUTH=${email}:${session.role}`); }
  const mongoUri = process.env.MONGODB_URI;
  if (!mongoUri) throw new Error("G6 runtime safety check failed: MONGODB_URI is required for read-only setup verification");
  await mongoose.connect(mongoUri, { serverSelectionTimeoutMS: 15_000 });
  try {
    const users = await User.find({ email: { $in: DISPOSABLE_EMAILS } }).select("email").lean(); const byEmail = new Map(users.map((user) => [user.email, user]));
    if (byEmail.size !== DISPOSABLE_EMAILS.length) fail("all six disposable identities must resolve");
    for (const email of ["dispose@1.com", "dispose@2.com", "dispose@3.com"] as const) {
      const wallets = await Wallet.find({ userId: byEmail.get(email)!._id }).select("currency availableBalance reservedBalance").lean();
    ...
    for (const email of ["dispose@4.com", "dispose@5.com", "dispose@6.com"] as const) {
      const services = await CreatorService.find({ creatorId: byEmail.get(email)!._id, isActive: true }).select("title currency price durationMinutes").lean();
    ...
    const usedSlots = new Set<string>(); const boundary = Date.now() + 24 * 60 * 60 * 1000;
    const findAvailableSlots = async (creatorId: mongoose.Types.ObjectId, serviceId: mongoose.Types.ObjectId, timing: Scenario["timing"]) => {
      const activeAvailabilities = await Availability.find({ creatorId, serviceId, status: "ACTIVE" }).select("_id").lean();
      ...
      return Slot.find({ creatorId, serviceId, availabilityId: { $in: availabilityIds }, status: "AVAILABLE", startTime }).sort({ startTime: 1 }).lean();
    };
    const createNonOverlappingAvailability = async (creator: DisposableEmail, creatorId: mongoose.Types.ObjectId, service: { _id: mongoose.Types.ObjectId; durationMinutes: number }, timing: Scenario["timing"]) => {
      const now = new Date();
      ...
      const existing = await Availability.find({ creatorId, serviceId: service._id, date: day, status: "ACTIVE" }).select("startTime endTime").lean();
      ...
      for (let startMinutes = 9 * 60; startMinutes + service.durationMinutes <= 24 * 60; startMinutes += service.durationMinutes) {
        ...
        const start = new Date(day.getTime() + startMinutes * 60_000);
        if (overlaps || start <= now) continue;
        const end = new Date(day.getTime() + endMinutes * 60_000);
        try {
          ...
          if (!message.includes("Availability overlaps with an existing active window")) throw error;
        }
      }
      fail(`no non-overlapping ${timing} availability window is available for ${creator}`);
    };
    type LiveScenario = {
      booking: G6BookingRead;
      payment: { status: string; amount: number; currency: string };
      reservation: { status: string; amount: number; currency: string };
      slots: Array<{ status: string; startTime: Date; endTime: Date }>;
      wallet: { currency: string; availableBalance: number; reservedBalance: number; currentBalance: number };
      expectedSlotStatus: "LOCKED" | "BOOKED";
      timingClassification: "REQUESTED" | "PROTECTED_ONGOING" | "PROTECTED_LT24" | "TERMINATE_GT24";
    };
    type ScenarioRead = { decision: "REUSE"; live: LiveScenario } | { decision: "REPLACE_HISTORICAL" | "REPLACE_TIME" | "REPLACE_SCENARIO"; reason: string };
    const historicalStatuses = new Set(["EXPIRED", "CANCELLED", "REJECTED", "COMPLETED"]);
    const inspectScenarioBooking = async (booking: G6BookingRead | null, scenario: Scenario, creatorId: mongoose.Types.ObjectId): Promise<ScenarioRead | null> => {
      if (!booking) return null;
      if (historicalStatuses.has(booking.status)) return { decision: "REPLACE_HISTORICAL", reason: `${booking.status} is historical for G6 live-scenario reuse` };
      ...
      const [payment, reservation, slots, wallet] = await Promise.all([
        ...
        BookingFundReservation.findOne({ bookingId: booking._id }).lean(),
        Slot.find({ _id: { $in: booking.slotIds } }).lean(),
        Wallet.findOne({ userId: booking.userId, currency: booking.currency }).select("currency availableBalance reservedBalance currentBalance").lean(),
      ]);
      ...
      if (booking.status !== expectedBookingStatus) return { decision: "REPLACE_SCENARIO", reason: `live booking status ${booking.status} does not satisfy ${scenario.timing}` };
      if (scenario.timing === "REQUESTED") return { decision: "REUSE", live: { booking, payment: livePayment, reservation: liveReservation, slots, wallet: liveWallet, expectedSlotStatus: "LOCKED", timingClassification: "REQUESTED" } };
      const now = Date.now();
      const allOngoing = slots.every((slot) => slot.startTime.getTime() <= now && slot.endTime.getTime() > now);
      const allLt24 = slots.every((slot) => slot.startTime.getTime() > now && slot.startTime.getTime() <= now + 24 * 60 * 60 * 1_000);
      const allGt24 = slots.every((slot) => slot.startTime.getTime() > now + 24 * 60 * 60 * 1_000);
      const timingClassification = allOngoing ? "PROTECTED_ONGOING" : allLt24 ? "PROTECTED_LT24" : allGt24 ? "TERMINATE_GT24" : null;
      if (!timingClassification) return { decision: "REPLACE_TIME", reason: "confirmed booking is no longer a current protected/greater-than-24-hour scenario" };
      ...
      if (!matches) return { decision: "REPLACE_TIME", reason: `current timing is ${timingClassification}` };
    ...
    const completedScenarios: Array<{ scenario: Scenario; bookingId: mongoose.Types.ObjectId; attemptIdentity: string | null }> = [];
    for (const scenario of setupScenarios) {
      const customer = byEmail.get(scenario.customer)!; const creator = byEmail.get(scenario.creator)!;
      const scenarioKeyBase = `g6-setup-booking-${scenario.label.toLowerCase()}`;
      let scenarioIdempotencyKey: string | undefined;
      let reusable: LiveScenario | null = null;
      const candidateKeys = [scenarioKeyBase, ...Array.from({ length: 24 }, (_, index) => `${scenarioKeyBase}-attempt-${index + 1}`)];
      for (const candidateKey of candidateKeys) {
        const identity = deriveBookingRequestIdentity({ userId: customer._id.toString(), serviceId: "", slotIds: [], method: PaymentMethod.WALLET, idempotencyKey: candidateKey });
        const keyedBooking = await Booking.findOne({ userId: customer._id, bookingRequestKey: identity.bookingRequestKey }).select("+bookingRequestKey").lean();
        if (!keyedBooking) {
          if (candidateKey !== scenarioKeyBase) { scenarioIdempotencyKey = candidateKey; break; }
          continue;
        }
        ...
        if (inspected) console.log(JSON.stringify({ scenarioDecision: { scenario: scenario.label, previousBookingReference: keyedBooking.bookingReference, previousStatus: keyedBooking.status, decision: inspected.decision, reason: inspected.reason } }));
      }
      if (!reusable) {
        const candidates = await Booking.find({ userId: customer._id, creatorId: creator._id }).sort({ createdAt: -1 }).limit(20).lean();
        for (const candidate of candidates) {
          const inspected = await inspectScenarioBooking(candidate, scenario, creator._id);
          if (inspected?.decision === "REUSE") { reusable = inspected.live; break; }
          if (inspected) console.log(JSON.stringify({ scenarioDecision: { scenario: scenario.label, previousBookingReference: candidate.bookingReference, previousStatus: candidate.status, decision: inspected.decision, reason: inspected.reason } }));
        }
      }
      if (reusable) {
      ...
      if (!scenarioIdempotencyKey) throw new Error(`G6 runtime safety check failed: no bounded replacement identity is available for ${scenario.label}`);
      const selectedService = await CreatorService.findOne({ creatorId: creator._id, isActive: true, currency: scenario.currency }).sort({ createdAt: 1 }).lean();
      if (!selectedService) throw new Error(`G6 runtime safety check failed: no active ${scenario.currency} service exists for ${scenario.creator}`);
      ...
      const slot = slots.find((candidate) => !usedSlots.has(String(candidate._id))); if (!slot) throw new Error(`G6 runtime safety check failed: no unused ${scenario.timing} slot for ${scenario.creator}`);
      ...
      if (!previewCurrency || previewCurrency !== service.currency || previewCurrency !== scenario.currency || !walletFunding || typeof grossFundingAmount !== "number") throw new Error(`G6 runtime safety check failed: pricing preview currency/readiness is invalid for ${scenario.label}`);
      const selectedWallet = await Wallet.findOne({ userId: customer._id, currency: previewCurrency }).select("currency availableBalance reservedBalance").lean();
      ...
      const booking = await Booking.findOne({ userId: customer._id, creatorId: creator._id, slotIds: slot._id }).sort({ createdAt: -1 }).lean(); if (!booking) throw new Error(`G6 runtime safety check failed: booking creation failed for ${scenario.label}`);
      if (scenario.timing !== "REQUESTED") await sessions.get(scenario.creator)!.post(`/api/v1/creator/bookings/${String(booking._id)}/decision`, { decision: "ACCEPT" });
      const [reloaded, payment, reservation, slotState] = await Promise.all([Booking.findById(booking._id).lean(), Payment.findOne({ bookingId: booking._id }).lean(), BookingFundReservation.findOne({ bookingId: booking._id }).lean(), Slot.findById(slot._id).lean()]);
      if (!reloaded || !payment || !reservation || !slotState) throw new Error(`G6 runtime safety check failed: readback missing for ${scenario.label}`);
      const expectedBookingStatus = scenario.timing === "REQUESTED" ? "REQUESTED" : "CONFIRMED";
      ...
      const wallet = await Wallet.findOne({ userId: customer._id, currency: reloaded.currency }).lean();
      if (!wallet) throw new Error(`G6 runtime safety check failed: customer Wallet readback missing for ${scenario.label}`);
    ...
    for (const { scenario, bookingId, attemptIdentity } of completedScenarios) {
      ...
      if (!inspected || inspected.decision !== "REUSE") throw new Error(`G6 runtime safety check failed: final readback failed for ${scenario.label}`);
...
const runCertification = async () => {
  ...
  const admin = await G6AdminApi.login(baseUrl);
  const mongoUri = process.env.MONGODB_URI;
  if (!mongoUri) throw new Error("G6 runtime safety check failed: MONGODB_URI is required for read-only certification verification");
  await mongoose.connect(mongoUri, { serverSelectionTimeoutMS: 15_000 });
  try {
    const users = await User.find({ email: { $in: DISPOSABLE_EMAILS } }).select("email governanceState status userCooldownUntil creatorCooldownUntil").lean();
    const byEmail = new Map(users.map((user) => [user.email, user]));
    if (byEmail.size !== DISPOSABLE_EMAILS.length) fail("all six disposable identities must resolve before certification");
    const ids = new Set(users.map((user) => user._id.toString()));
    const snapshot = async (stage: string, target: DisposableEmail) => {
      const targetIdentity = byEmail.get(target)!;
      const targetUser = await User.findById(targetIdentity._id)
        .select("email governanceState status userCooldownUntil creatorCooldownUntil")
        .lean();
      if (!targetUser) throw new Error(`G6 runtime safety check failed: target disappeared during ${stage}`);
      const bookings = await Booking.find({ $or: [{ userId: targetUser._id }, { creatorId: targetUser._id }] }).lean();
      for (const booking of bookings) {
        if (!ids.has(booking.userId.toString()) || !ids.has(booking.creatorId.toString())) fail(`non-whitelisted booking participant detected for ${target}`);
      }
      const bookingRows = await Promise.all(bookings.map(async (booking) => {
        const [payment, reservation, slots] = await Promise.all([Payment.findOne({ bookingId: booking._id }).lean(), BookingFundReservation.findOne({ bookingId: booking._id }).lean(), Slot.find({ _id: { $in: booking.slotIds } }).lean()]);
      ...
      const wallets = await Wallet.find({ userId: targetUser._id }).select("currency availableBalance reservedBalance").lean();
    ...
    const execute = async (key: "SUSPEND_USER" | "ACTIVATE_USER" | "BAN_USER" | "RESET_USER_TRUST" | "APPLY_CREATOR_COOLDOWN" | "REVOKE_CREATOR_COOLDOWN", target: DisposableEmail, params: JsonRecord = {}) => {
      ...
      const targetId = key === "APPLY_CREATOR_COOLDOWN" || key === "REVOKE_CREATOR_COOLDOWN"
        ? (await CreatorProfile.findOne({ userId: user._id, status: "active" }).select("_id").lean())?._id.toString()
        : user._id.toString();
      if (!targetId) throw new Error(`G6 runtime safety check failed: active Creator profile is required for ${key} on ${target}`);
      ...
      if (!confirmationToken) throw new Error(`G6 runtime safety check failed: dry-run did not return a confirmation token for ${key} on ${target}`);
    ...
    const replay = async (key: "SUSPEND_USER" | "BAN_USER" | "REVOKE_CREATOR_COOLDOWN", target: DisposableEmail, action: { targetId: string; reason: string; confirmationToken: string; normalizedParams: JsonRecord }) => {
      ...
      if (!isRecord(replayed) || replayed.replay !== true) fail(`${key} replay did not return the authoritative replay result`);
    ...
    await admin.get("/api/v1/admin/actions/logs"); await admin.get("/api/v1/admin/audit-logs");
    ...
    try { await execute("ACTIVATE_USER", "dispose@2.com"); } catch (error) { bannedUserActivationBlocked = true; console.log(JSON.stringify({ expectedBlockedAction: { key: "ACTIVATE_USER", target: "dispose@2.com", error: error instanceof Error ? error.message : "blocked" } })); }
    if (!bannedUserActivationBlocked) fail("ordinary activation unexpectedly succeeded for banned user");
    await execute("RESET_USER_TRUST", "dispose@2.com");
    await execute("SUSPEND_USER", "dispose@4.com"); await execute("ACTIVATE_USER", "dispose@4.com");
    const bannedCreator = await execute("BAN_USER", "dispose@5.com");
    let bannedCreatorActivationBlocked = false;
    try { await execute("ACTIVATE_USER", "dispose@5.com"); } catch (error) { bannedCreatorActivationBlocked = true; console.log(JSON.stringify({ expectedBlockedAction: { key: "ACTIVATE_USER", target: "dispose@5.com", error: error instanceof Error ? error.message : "blocked" } })); }
    if (!bannedCreatorActivationBlocked) fail("ordinary activation unexpectedly succeeded for banned Creator");
    ...
    if (!mismatchToken) throw new Error("G6 runtime safety check failed: confirmation mismatch dry-run did not return a token");
    let mismatchBlocked = false;
    try { await admin.executeConfirmedGovernanceAction({ key: "SUSPEND_USER", target: { email: "dispose@1.com", id: byEmail.get("dispose@1.com")!._id.toString() }, params: {}, reason: "G6 confirmation mismatch proof", confirmationToken: mismatchToken }); } catch (error) { mismatchBlocked = true; console.log(JSON.stringify({ confirmationMismatch: { rejected: true, error: error instanceof Error ? error.message : "rejected" } })); }
    if (!mismatchBlocked) fail("confirmation mismatch unexpectedly executed");
    await admin.get("/api/v1/admin/actions/logs"); await admin.get("/api/v1/admin/audit-logs");
...
const runTargetedCertification = async () => {
  const baseUrl = assertSafeRuntime();
  const missing = [...TARGETED_SESSION_EMAILS, "admin@test.com"].filter((email) => email === "admin@test.com" ? !process.env.G6_ADMIN_PASSWORD : !process.env[PASSWORD_VARIABLES[email as DisposableEmail]]);
  if (missing.length) fail(`missing required authenticated runtime credentials for: ${missing.join(", ")}`);
  const sessions = new Map<DisposableEmail, DisposableSession>();
  for (const email of TARGETED_SESSION_EMAILS) sessions.set(email, await DisposableSession.login(baseUrl, email, process.env[PASSWORD_VARIABLES[email]]!));
  const admin = await G6AdminApi.login(baseUrl);
  const mongoUri = process.env.MONGODB_URI;
  if (!mongoUri) throw new Error("G6 runtime safety check failed: MONGODB_URI is required for read-only targeted certification readback");
  await mongoose.connect(mongoUri, { serverSelectionTimeoutMS: 15_000 });
  try {
    type TargetBookingState = {
      booking: G6BookingRead & { bookingReference: string };
      payment: { status: string; amount: number; currency: string };
      reservation: { status: string; amount: number; currency: string };
      slots: Array<{ status: string; startTime: Date; endTime: Date }>;
      wallet: { currency: string; availableBalance: number; reservedBalance: number };
      releases: Array<{ direction: string; account: string; amount: number; currency: string; transactionId?: string }>;
    };
    const users = await User.find({ email: { $in: DISPOSABLE_EMAILS } }).select("email role status governanceState userCooldownUntil creatorCooldownUntil creatorCooldownReason creatorCooldownTriggeredAt").lean();
    const byEmail = new Map(users.map((user) => [user.email, user]));
    if (byEmail.size !== DISPOSABLE_EMAILS.length) fail("all six disposable identities must resolve");
    for (const email of ["dispose@1.com", "dispose@3.com", "dispose@4.com"] as const) {
      const user = byEmail.get(email)!;
      if (user.governanceState !== "ACTIVE" || user.status !== "active" || user.userCooldownUntil || user.creatorCooldownUntil) fail(`${email} must be ACTIVE with no cooldown before targeted certification`);
    }
    const cooldownTarget = byEmail.get("dispose@6.com")!;
    const cooldownProfile = await CreatorProfile.findOne({ userId: cooldownTarget._id, status: "active" }).select("_id creatorCooldownUntil").lean();
    if (!cooldownProfile || cooldownTarget.governanceState !== "ACTIVE" || cooldownTarget.status !== "active" || cooldownTarget.userCooldownUntil) throw new Error("G6 runtime safety check failed: dispose@6.com must remain ACTIVE with no user cooldown before targeted certification");
    ...
    const [targetedCooldownAction, targetedCooldownAudit] = cooldownIsActive
      ? await Promise.all([
        AdminActionLog.findOne({ actionKey: "APPLY_CREATOR_COOLDOWN", targetId: cooldownProfile._id, status: "SUCCESS", reason: targetedCooldownReason }).sort({ createdAt: -1 }).lean(),
        AuditLog.findOne({ action: "CREATOR_COOLDOWN_APPLIED", entityType: "USER", entityId: cooldownTarget._id, "after.reason": targetedCooldownReason }).sort({ createdAt: -1 }).lean(),
      ])
      : [null, null];
    ...
    if (cooldownIsActive && !resumeCooldown) fail("dispose@6.com has an active Creator cooldown not safely attributable to this targeted G6 run");
    ...
    const liveRead = async (bookingId: mongoose.Types.ObjectId): Promise<TargetBookingState> => {
      const booking = await Booking.findById(bookingId).lean();
      if (!booking) throw new Error("G6 runtime safety check failed: booking disappeared during targeted certification");
      const [payment, reservation, slots, wallet, releases] = await Promise.all([
        Payment.findOne({ bookingId }).lean(), BookingFundReservation.findOne({ bookingId }).lean(), Slot.find({ _id: { $in: booking.slotIds } }).lean(),
        Wallet.findOne({ userId: booking.userId, currency: booking.currency }).select("currency availableBalance reservedBalance").lean(),
        LedgerEntry.find({ bookingId, source: "BOOKING_WALLET_RESERVATION_RELEASE" }).select("direction account amount currency transactionId").lean(),
      ]);
      if (!payment || !reservation || !wallet) throw new Error(`G6 runtime safety check failed: missing financial authority for ${booking.bookingReference}`);
      if (!booking.bookingReference || typeof booking.totalAmount !== "number" || !booking.currency || !payment.status || typeof payment.amount !== "number" || !payment.currency || !reservation.status || typeof reservation.amount !== "number" || !reservation.currency || !wallet.currency || typeof wallet.availableBalance !== "number" || typeof wallet.reservedBalance !== "number" || slots.some((slot) => !slot.status || !slot.startTime || !slot.endTime) || releases.some((entry) => !entry.direction || !entry.account || typeof entry.amount !== "number" || !entry.currency)) throw new Error(`G6 runtime safety check failed: incomplete authoritative read for ${bookingId}`);
      return {
    ...
    const assertLive = async (bookingId: mongoose.Types.ObjectId, expected: "REQUESTED" | "CONFIRMED") => {
      const state = await liveRead(bookingId);
      const slotStatus = expected === "REQUESTED" ? "LOCKED" : "BOOKED";
      if (state.booking.status !== expected || state.payment.status !== "AUTHORIZED" || state.reservation.status !== "ACTIVE" || state.slots.some((slot) => slot.status !== slotStatus) || state.wallet.reservedBalance < state.reservation.amount) fail(`live baseline failed for ${state.booking.bookingReference}`);
      return state;
    };
    const ensureSlot = async (creator: DisposableEmail, creatorId: mongoose.Types.ObjectId, service: { _id: mongoose.Types.ObjectId; durationMinutes: number }, timing: "REQUESTED" | "LT24" | "GT24") => {
      const now = new Date(); const boundary = now.getTime() + 24 * 60 * 60 * 1000;
      const search = async () => {
        const availabilityIds = (await Availability.find({ creatorId, serviceId: service._id, status: "ACTIVE" }).select("_id").lean()).map((row) => row._id);
        return Slot.find({ creatorId, serviceId: service._id, availabilityId: { $in: availabilityIds }, status: "AVAILABLE", startTime: timing === "GT24" ? { $gt: new Date(boundary) } : { $gt: now, $lte: new Date(boundary) } }).sort({ startTime: 1 }).lean();
      };
      let slots = await search(); let slot = slots.find((row) => !usedSlots.has(String(row._id)));
      if (!slot) {
        const day = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + (timing === "GT24" ? 2 : 1)));
        const start = new Date(day.getTime() + 9 * 60 * 60 * 1000); const end = new Date(start.getTime() + service.durationMinutes * 60_000);
        await sessions.get(creator)!.post("/api/v1/creator/availability", { serviceId: String(service._id), date: start.toISOString().slice(0, 10), startTime: start.toISOString().slice(11, 16), endTime: end.toISOString().slice(11, 16), timezone: "UTC" });
        slots = await search(); slot = slots.find((row) => !usedSlots.has(String(row._id)));
      }
      if (!slot) throw new Error(`G6 runtime safety check failed: no safe ${timing} slot for ${creator}`); usedSlots.add(String(slot._id)); return slot;
    };
    const create = async (label: string, customer: DisposableEmail, creator: DisposableEmail, timing: "REQUESTED" | "LT24" | "GT24"): Promise<TargetBookingState> => {
      const customerUser = byEmail.get(customer)!; const creatorUser = byEmail.get(creator)!;
      const services = await CreatorService.find({ creatorId: creatorUser._id, isActive: true }).sort({ createdAt: 1 }).lean();
      for (const service of services) {
        if (!service.currency || typeof service.durationMinutes !== "number") continue;
        const wallet = await Wallet.findOne({ userId: customerUser._id, currency: service.currency }).select("availableBalance reservedBalance currency").lean();
        if (!wallet) continue;
        ...
        if (!preview || preview.currency !== service.currency || !isRecord(preview.walletFunding) || preview.walletFunding.sufficient !== true || required === null || wallet.availableBalance < required) continue;
        let booking: G6BookingRead | null = null;
        for (let attempt = 1; attempt <= 12 && !booking; attempt += 1) {
          const key = `g6-targeted-${label.toLowerCase()}-attempt-${attempt}`;
          const identity = deriveBookingRequestIdentity({ userId: String(customerUser._id), serviceId: String(service._id), slotIds: [String(slot._id)], method: PaymentMethod.WALLET, idempotencyKey: key });
          const existing = await Booking.findOne({ userId: customerUser._id, bookingRequestKey: identity.bookingRequestKey }).lean();
          ...
          booking = await Booking.findOne({ userId: customerUser._id, bookingRequestKey: identity.bookingRequestKey }).lean();
        }
        if (!booking) throw new Error(`G6 runtime safety check failed: no bounded booking identity available for ${label}`);
        if (timing !== "REQUESTED") await sessions.get(creator)!.post(`/api/v1/creator/bookings/${String(booking._id)}/decision`, { decision: "ACCEPT" });
    ...
    const invoke = async (key: "BAN_USER" | "ACTIVATE_USER" | "APPLY_CREATOR_COOLDOWN" | "REVOKE_CREATOR_COOLDOWN", target: DisposableEmail) => {
      const user = byEmail.get(target)!;
      const creatorProfile = key.includes("COOLDOWN") ? await CreatorProfile.findOne({ userId: user._id, status: "active" }).select("_id").lean() : null;
      const targetId: string = key.includes("COOLDOWN") ? (creatorProfile ? String(creatorProfile._id) : "") : String(user._id);
      if (!targetId) throw new Error(`G6 runtime safety check failed: missing active Creator profile for ${key}`);
      ...
      const token = getString(dry, "confirmationToken"); if (!token) throw new Error(`G6 runtime safety check failed: dry-run did not issue confirmation for ${key}`);
    ...
    const replay = async (key: "BAN_USER" | "REVOKE_CREATOR_COOLDOWN", target: DisposableEmail, action: Awaited<ReturnType<typeof invoke>>) => {
      ...
      if (!isRecord(result) || result.replay !== true) fail(`${key} replay did not return replay=true`);
    };
    const assertBlockedBooking = async (label: string, customer: DisposableEmail, creator: DisposableEmail) => {
      const customerUser = byEmail.get(customer)!; const creatorUser = byEmail.get(creator)!;
      const services = await CreatorService.find({ creatorId: creatorUser._id, isActive: true }).sort({ createdAt: 1 }).lean();
      const customerWallets = await Wallet.find({ userId: customerUser._id, availableBalance: { $gt: 0 } }).select("currency availableBalance reservedBalance").lean();
      const walletByCurrency = new Map<string, (typeof customerWallets)[number]>(customerWallets.filter((wallet) => Boolean(wallet.currency)).map((wallet) => [wallet.currency!, wallet]));
      ...
      const slot = await ensureSlot(creator, creatorUser._id, service, "GT24"); const key = `g6-targeted-blocked-${label.toLowerCase()}`;
      const identity = deriveBookingRequestIdentity({ userId: String(customerUser._id), serviceId: String(service._id), slotIds: [String(slot._id)], method: PaymentMethod.WALLET, idempotencyKey: key });
      const before = walletByCurrency.get(service.currency)!;
      let blocked = false; let error = "";
      try { await sessions.get(customer)!.post("/api/v1/bookings/request", { serviceId: String(service._id), slotIds: [String(slot._id)], paymentMethod: "WALLET" }, { "Idempotency-Key": key }); } catch (cause) { blocked = true; error = cause instanceof Error ? cause.message : "blocked"; }
      const [after, booking, slotState] = await Promise.all([Wallet.findOne({ userId: customerUser._id, currency: service.currency }).select("availableBalance reservedBalance").lean(), Booking.findOne({ userId: customerUser._id, bookingRequestKey: identity.bookingRequestKey }).lean(), Slot.findById(slot._id).lean()]);
      if (!blocked || booking || !after || before.availableBalance !== after.availableBalance || before.reservedBalance !== after.reservedBalance || slotState?.status !== "AVAILABLE") fail(`blocked booking residue or unexpected success for ${label}; httpError=${error || "request unexpectedly succeeded"}`);
    ...
    let cdExisting: TargetBookingState;
    let cdAccept: TargetBookingState;
    if (resumeCooldown) {
      const [existingBooking, acceptanceBooking] = await Promise.all([
        Booking.findOne({ bookingReference: "BKG-3CDD807659F52E0E61A32249" }).select("_id").lean(),
        Booking.findOne({ bookingReference: "BKG-E9DB7AD58EE868FD7909379E" }).select("_id").lean(),
      ]);
      if (!existingBooking || !acceptanceBooking) throw new Error("G6 runtime safety check failed: targeted cooldown resume baselines are missing; do not recreate while cooldown is active");
      cdExisting = await assertLive(existingBooking._id, "CONFIRMED");
      cdAccept = await assertLive(acceptanceBooking._id, "REQUESTED");
      if (cdExisting.booking.creatorId.toString() !== cooldownTarget._id.toString() || cdAccept.booking.creatorId.toString() !== cooldownTarget._id.toString()) fail("targeted cooldown resume baselines do not belong to dispose@6.com");
      ...
      const cdBefore = await liveRead(cdExisting.booking._id); await invoke("APPLY_CREATOR_COOLDOWN", "dispose@6.com");
      const cdAfter = await liveRead(cdExisting.booking._id);
      if (cdBefore.booking.status !== cdAfter.booking.status || cdBefore.payment.status !== cdAfter.payment.status || cdBefore.reservation.status !== cdAfter.reservation.status || cdBefore.wallet.availableBalance !== cdAfter.wallet.availableBalance || cdBefore.wallet.reservedBalance !== cdAfter.wallet.reservedBalance || cdBefore.slots.some((slot, index) => slot.status !== cdAfter.slots[index]?.status) || cdAfter.releases.length !== cdBefore.releases.length) fail("Creator cooldown changed an existing booking or financial state");
      await assertBlockedBooking("COOLDOWN_INCOMING", "dispose@3.com", "dispose@6.com");
    }
    let acceptanceBlocked = false; try { await sessions.get("dispose@6.com")!.post(`/api/v1/creator/bookings/${String(cdAccept.booking._id)}/decision`, { decision: "ACCEPT" }); } catch { acceptanceBlocked = true; }
    if (!acceptanceBlocked) fail("Creator cooldown did not block acceptance"); await assertLive(cdAccept.booking._id, "REQUESTED");
    await assertBlockedBooking("COOLDOWN_OUTGOING", "dispose@6.com", "dispose@4.com");
    const revoke = await invoke("REVOKE_CREATOR_COOLDOWN", "dispose@6.com"); await replay("REVOKE_CREATOR_COOLDOWN", "dispose@6.com", revoke);
    const cooldownUser = await User.findById(byEmail.get("dispose@6.com")!._id).select("governanceState status creatorCooldownUntil userCooldownUntil").lean();
    if (!cooldownUser || cooldownUser.governanceState !== "ACTIVE" || cooldownUser.status !== "active" || cooldownUser.creatorCooldownUntil) fail("Creator cooldown revoke did not restore canonical state");
    await sessions.get("dispose@6.com")!.post(`/api/v1/creator/bookings/${String(cdAccept.booking._id)}/decision`, { decision: "ACCEPT" }); await assertLive(cdAccept.booking._id, "CONFIRMED");
    ...
    await assertLive(cbr1.booking._id, "REQUESTED"); await assertLive(cbr2.booking._id, "CONFIRMED"); await assertLive(cbr3.booking._id, "CONFIRMED");
    const ban = await invoke("BAN_USER", "dispose@4.com"); await replay("BAN_USER", "dispose@4.com", ban);
    const [after1, after2, after3, bannedUser] = await Promise.all([liveRead(cbr1.booking._id), liveRead(cbr2.booking._id), liveRead(cbr3.booking._id), User.findById(byEmail.get("dispose@4.com")!._id).select("governanceState status").lean()]);
    const released = (state: Awaited<ReturnType<typeof liveRead>>) => state.booking.status === "CANCELLED" && state.payment.status === "CANCELLED" && state.reservation.status === "RELEASED" && state.slots.every((slot) => slot.status === "AVAILABLE") && state.releases.length === 2 && state.releases.some((entry) => entry.direction === "DEBIT" && entry.account === "WALLET_RESERVED" && entry.amount === state.reservation.amount) && state.releases.some((entry) => entry.direction === "CREDIT" && entry.account === "WALLET_AVAILABLE" && entry.amount === state.reservation.amount);
    if (!bannedUser || bannedUser.governanceState !== "BANNED" || bannedUser.status !== "banned" || !released(after1) || !released(after3) || after2.booking.status !== "CONFIRMED" || after2.payment.status !== "AUTHORIZED" || after2.reservation.status !== "ACTIVE" || after2.slots.some((slot) => slot.status !== "BOOKED") || after2.releases.length) fail("Creator ban matrix failed authoritative postconditions");
    ...
    if (!isRecord(activateDry) || activateDry.outcome !== "BLOCKED") fail("ordinary activation did not return an explicit blocked outcome for banned Creator");
    const [actions, audits] = await Promise.all([AdminActionLog.find({ actionKey: { $in: ["BAN_USER", "APPLY_CREATOR_COOLDOWN", "REVOKE_CREATOR_COOLDOWN"] } }).lean(), AuditLog.find({ entityType: "USER", entityId: { $in: [byEmail.get("dispose@4.com")!._id, byEmail.get("dispose@6.com")!._id] } }).lean()]);
    if (!actions.length || !audits.length) fail("targeted action/audit log readback missing");
...
const runFinishRemainingCertification = async () => {
  ...
  if (!mongoUri) fail("MONGODB_URI is required for read-only cooldown certification readback");
  await mongoose.connect(mongoUri!, { serverSelectionTimeoutMS: 15_000 });
  try {
    const cooldownReason = "G6 targeted runtime certification: APPLY_CREATOR_COOLDOWN for dispose@6.com";
    const u6 = await User.findOne({ email: "dispose@6.com" })
      ...
      .lean();
    const u6Profile = u6
      ? await CreatorProfile.findOne({ userId: u6._id, status: "active" }).select("_id creatorCooldownUntil").lean()
      : null;
    if (!u6 || !u6Profile) {
    ...
    const [applyLog, applyAudit] = await Promise.all([
      AdminActionLog.findOne({ actionKey: "APPLY_CREATOR_COOLDOWN", targetId: targetU6Profile._id, status: "SUCCESS", reason: cooldownReason }).lean(),
      AuditLog.findOne({ action: "CREATOR_COOLDOWN_APPLIED", entityType: "USER", entityId: targetU6._id, "after.reason": cooldownReason }).lean(),
    ]);
    if (!applyLog || !applyAudit) fail("active U6 cooldown is not safely attributable to the targeted G6 run");

    type CooldownBookingState = {
      reference: string;
      status: string;
      payment: string;
      reservation: string;
    ...
    const readBooking = async (reference: string): Promise<{ id: mongoose.Types.ObjectId; creatorId: mongoose.Types.ObjectId; state: CooldownBookingState }> => {
      const booking = await Booking.findOne({ bookingReference: reference }).lean();
      if (!booking) fail(`cooldown baseline booking is missing: ${reference}`);
      const safeBooking = booking!;
      const [payment, reservation, slots, wallet, releases, ledgerEntryCount] = await Promise.all([
        Payment.findOne({ bookingId: safeBooking._id }).select("status").lean(),
        BookingFundReservation.findOne({ bookingId: safeBooking._id }).select("status amount").lean(),
        Slot.find({ _id: { $in: safeBooking.slotIds } }).select("status").lean(),
        Wallet.findOne({ userId: safeBooking.userId, currency: safeBooking.currency }).select("availableBalance reservedBalance").lean(),
        LedgerEntry.find({ bookingId: safeBooking._id, source: "BOOKING_WALLET_RESERVATION_RELEASE" }).select("_id").lean(),
        LedgerEntry.countDocuments({ bookingId: safeBooking._id }),
      ]);
      if (!payment || !reservation || !wallet || typeof reservation.amount !== "number" || typeof wallet.availableBalance !== "number" || typeof wallet.reservedBalance !== "number") fail(`cooldown baseline financial read is incomplete: ${reference}`);
      return {
        id: safeBooking._id,
    ...
    const assertBaseline = (state: CooldownBookingState, expected: "REQUESTED" | "CONFIRMED") => {
    ...
    if (existingBefore.creatorId.toString() !== targetU6._id.toString() || acceptanceBefore.creatorId.toString() !== targetU6._id.toString()) fail("fixed cooldown baselines are not owned by dispose@6.com");
    ...
    if (!acceptanceError) fail("Creator cooldown did not block acceptance");
    ...
    if (JSON.stringify(existingBefore.state) !== JSON.stringify(existingAfterBlocked.state)) fail("Creator cooldown acceptance attempt changed the existing booking baseline");
    if (JSON.stringify(acceptanceBefore.state) !== JSON.stringify(acceptanceAfterBlocked.state)) fail("Creator cooldown acceptance attempt changed the pending booking baseline");

    const revokeReason = "G6 targeted runtime certification: REVOKE_CREATOR_COOLDOWN for dispose@6.com";
    const revokeAuditCountBefore = await AuditLog.countDocuments({ action: "CREATOR_COOLDOWN_REVOKED", entityType: "USER", entityId: targetU6._id });
    ...
    const confirmationToken = getString(dry, "confirmationToken");
    if (!confirmationToken) fail("REVOKE_CREATOR_COOLDOWN dry-run did not return a confirmation token");
    ...
    if (!isRecord(replay) || replay.replay !== true) fail("REVOKE_CREATOR_COOLDOWN replay did not return replay=true");
    const [u6After, profileAfter, existingAfterRevoke, acceptanceAfterRevoke, revokeAuditCountAfter, revokeLogs] = await Promise.all([
      User.findById(targetU6._id).select("status governanceState userCooldownUntil creatorCooldownUntil").lean(),
      CreatorProfile.findById(targetU6Profile._id).select("creatorCooldownUntil").lean(),
      readBooking(existingBefore.state.reference),
      readBooking(acceptanceBefore.state.reference),
      AuditLog.countDocuments({ action: "CREATOR_COOLDOWN_REVOKED", entityType: "USER", entityId: targetU6._id }),
      AdminActionLog.find({ actionKey: "REVOKE_CREATOR_COOLDOWN", targetId: targetU6Profile._id, status: "SUCCESS", reason: revokeReason }).lean(),
    ]);
    if (!u6After || !profileAfter || u6After.governanceState !== "ACTIVE" || u6After.status !== "active" || u6After.userCooldownUntil || u6After.creatorCooldownUntil || profileAfter.creatorCooldownUntil || revokeAuditCountAfter !== revokeAuditCountBefore + 1 || revokeLogs.length !== 1) fail("Creator cooldown revoke did not preserve the exact canonical/replay-safe result");
    assertBaseline(existingAfterRevoke.state, "CONFIRMED");
    assertBaseline(acceptanceAfterRevoke.state, "REQUESTED");
    if (JSON.stringify(existingBefore.state) !== JSON.stringify(existingAfterRevoke.state)) fail("Creator cooldown revoke changed the existing booking or financial baseline");
    await u6Session.post(`/api/v1/creator/bookings/${String(acceptanceBefore.id)}/decision`, { decision: "ACCEPT" });
    const acceptanceRestored = await readBooking(acceptanceBefore.state.reference);
    if (acceptanceRestored.state.status !== "CONFIRMED" || acceptanceRestored.state.payment !== "AUTHORIZED" || acceptanceRestored.state.reservation !== "ACTIVE" || acceptanceRestored.state.slotStates.some((status) => status !== "BOOKED") || acceptanceRestored.state.releaseEntries !== 0) fail("Creator acceptance was not restored after cooldown revoke");

    const [activeUsers, activeProfiles] = await Promise.all([
      User.find({ email: { $in: DISPOSABLE_EMAILS }, governanceState: "ACTIVE", status: "active" }).select("_id email").lean(),
      CreatorProfile.find({ status: "active" }).select("userId").lean(),
    ]);
    const activeProfileUserIds = new Set(activeProfiles.map((profile) => String(profile.userId)));
    const activeCreatorEmails = activeUsers.filter((user) => activeProfileUserIds.has(String(user._id))).map((user) => user.email).filter((email): email is string => typeof email === "string").sort();
...
const runFinalBannedCreatorCustomerProof = async () => {
  ...
  try {
    const [rawU6, rawU7] = await Promise.all([
      User.findOne({ email: "dispose@6.com" }).select("email role status governanceState userCooldownUntil creatorCooldownUntil").lean(),
      User.findOne({ email: FINAL_BAN_GATE_COUNTERPARTY }).select("email role status governanceState creatorStatus userCooldownUntil creatorCooldownUntil").lean(),
    ]);
    if (!rawU6 || !rawU7) {
    ...
    const [u7UserProfile, u7CreatorProfile, services, u6Wallets] = await Promise.all([
      UserProfile.findOne({ userId: u7._id }).select("profileStatus").lean(),
      CreatorProfile.findOne({ userId: u7._id, status: "active" }).select("_id status slug creatorCooldownUntil").lean(),
      CreatorService.find({ creatorId: u7._id, isActive: true }).sort({ createdAt: 1 }).lean(),
      Wallet.find({ userId: u6._id }).select("currency availableBalance reservedBalance lockedBalance currentBalance").lean(),
    ]);
    ...
    type FinalCandidate = { service: (typeof services)[number]; currency: SupportedCurrency; wallet: (typeof u6Wallets)[number] | undefined; serviceAmount: number; customerFeeAmount: number; grossFundingAmount: number };
    const candidates: FinalCandidate[] = [];
    for (const service of services) {
      if (!service.currency || !SUPPORTED_CURRENCIES.includes(service.currency as SupportedCurrency) || typeof service.price !== "number" || typeof service.durationMinutes !== "number") continue;
      const currency = service.currency as SupportedCurrency;
      const serviceAmount = creatorServiceMajorToMinor(service.price, currency);
      const pricing = marketplacePricingService.calculate({ serviceAmount, currency });
    ...
    const findAvailableSlot = async () => Slot.findOne({ creatorId: u7._id, serviceId: service._id, status: "AVAILABLE", startTime: { $gt: new Date() } }).sort({ startTime: 1 }).lean();
    ...
    if (!slot) fail("no valid future U7 slot could be established through the normal availability authority; U6 was not banned");
    const safeSlot = slot!;
    ...
    const identity = deriveBookingRequestIdentity({ userId: String(u6._id), serviceId: String(service._id), slotIds: [String(safeSlot._id)], method: PaymentMethod.WALLET, idempotencyKey });
    const preexistingAttempt = await Booking.findOne({ userId: u6._id, bookingRequestKey: identity.bookingRequestKey }).select("_id").lean();
    if (preexistingAttempt) fail("the final ban-gate idempotency identity already has a Booking; U6 was not banned");
    const snapshotExistingBookings = async () => {
      const bookings = await Booking.find({ $or: [{ userId: u6._id }, { creatorId: u6._id }] }).select("bookingReference status userId creatorId slotIds").sort({ createdAt: 1 }).lean();
      const rows = [];
      for (const booking of bookings) {
        const [payment, reservation, slots, releases] = await Promise.all([
          Payment.findOne({ bookingId: booking._id }).select("status").lean(),
          BookingFundReservation.findOne({ bookingId: booking._id }).select("status amount currency").lean(),
          Slot.find({ _id: { $in: booking.slotIds } }).select("status").lean(),
          LedgerEntry.find({ bookingId: booking._id, source: "BOOKING_WALLET_RESERVATION_RELEASE" }).select("direction account amount currency").lean(),
        ]);
    ...
    const confirmationToken = getString(dry, "confirmationToken");
    if (!confirmationToken) fail("BAN_USER dry-run did not issue confirmation; U6 was not banned");
    ...
    if (!isRecord(replay) || replay.replay !== true) fail("BAN_USER replay did not return replay=true");
    const existingAfterReplay = await snapshotExistingBookings();
    if (JSON.stringify(existingAfterBan) !== JSON.stringify(existingAfterReplay)) fail("BAN_USER replay changed existing booking/financial consequences");
    const [u6Banned, banLogs, banAudits, walletBeforeAttempt, slotBeforeAttempt, authorizationLedgerBefore] = await Promise.all([
      User.findById(u6._id).select("status governanceState").lean(),
      AdminActionLog.find({ actionKey: "BAN_USER", targetId: u6._id, status: "SUCCESS", reason }).lean(),
      AuditLog.find({ action: "USER_BANNED", entityType: "USER", entityId: u6._id }).lean(),
      Wallet.findOne({ userId: u6._id, currency }).select("availableBalance reservedBalance lockedBalance currentBalance").lean(),
      Slot.findById(safeSlot._id).select("status").lean(),
      LedgerEntry.countDocuments({ userId: u6._id, source: "BOOKING_WALLET_AUTHORIZATION", createdAt: { $gte: new Date() } }),
    ]);
    ...
    const [walletAfterAttempt, slotAfterAttempt, attemptedBooking, authorizationLedgerAfter] = await Promise.all([
      Wallet.findOne({ userId: u6._id, currency }).select("availableBalance reservedBalance lockedBalance currentBalance").lean(),
      Slot.findById(safeSlot._id).select("status").lean(),
      Booking.findOne({ userId: u6._id, bookingRequestKey: identity.bookingRequestKey }).lean(),
      LedgerEntry.countDocuments({ userId: u6._id, source: "BOOKING_WALLET_AUTHORIZATION", createdAt: { $gte: attemptStartedAt } }),
    ]);
...
const runFundU6Inr = async () => {
  const baseUrl = assertSafeRuntime();
  if (!process.env.G6_CREATOR6_PASSWORD || !process.env.G6_ADMIN_PASSWORD) throw new Error("G6 runtime safety check failed: G6_CREATOR6_PASSWORD and G6_ADMIN_PASSWORD are required");
  const [u6Session, admin] = await Promise.all([
    DisposableSession.login(baseUrl, "dispose@6.com", process.env.G6_CREATOR6_PASSWORD),
    G6AdminApi.login(baseUrl),
  ]);
  const mongoUri = process.env.MONGODB_URI;
  if (!mongoUri) throw new Error("G6 runtime safety check failed: MONGODB_URI is required for U6 INR funding verification");
  await mongoose.connect(mongoUri, { serverSelectionTimeoutMS: 15_000 });
  try {
    const [u6, u4] = await Promise.all([User.findOne({ email: "dispose@6.com" }).select("email governanceState status creatorCooldownUntil").lean(), User.findOne({ email: "dispose@4.com" }).select("email").lean()]);
    if (!u6 || !u4 || u6.governanceState !== "ACTIVE" || u6.status !== "active") throw new Error("G6 runtime safety check failed: U6 must remain canonically ACTIVE for Wallet funding");
    const service = await CreatorService.findOne({ creatorId: u4._id, isActive: true, currency: "INR" }).sort({ createdAt: 1 }).lean();
    if (!service) throw new Error("G6 runtime safety check failed: no active INR service exists for dispose@4.com");
    const serviceAmount = creatorServiceMajorToMinor(service.price, "INR");
    const requiredAmount = marketplacePricingService.calculate({ serviceAmount, currency: "INR" }).totalAmount;
    const wallet = await Wallet.findOne({ userId: u6._id, currency: "INR" }).select("currency availableBalance reservedBalance lockedBalance currentBalance").lean();
    ...
    const topUps = await WalletTopUpRequest.find({ userId: u6._id, currency: "INR" }).sort({ requestedAt: -1 }).lean();
    const completed = topUps.find((topUp) => topUp.status === "COMPLETED");
    if (completed) fail(`completed U6 INR top-up ${completed.topUpReference} does not have a sufficient Wallet projection`);
    const failed = topUps.find((topUp) => topUp.status === "FAILED" || topUp.status === "REJECTED");
    if (failed) fail(`existing U6 INR top-up ${failed.topUpReference} is ${failed.status}; do not create a duplicate without review`);
    type TopUpRead = { _id: mongoose.Types.ObjectId; topUpReference: string; amount: number; status: string };
    let topUp: TopUpRead | null = (topUps.find((item) => ["PENDING", "APPROVED", "PROCESSING"].includes(item.status)) as unknown as TopUpRead | undefined) ?? null;
    ...
    if (!topUp) {
      ...
      if (!reference) fail("top-up create did not return a safe reference");
      topUp = (await WalletTopUpRequest.findOne({ userId: u6._id, topUpReference: reference }).lean() as unknown as TopUpRead | null);
    }
    if (!topUp) throw new Error("G6 runtime safety check failed: U6 INR top-up request could not be read back");
    if (topUp.amount !== requiredAmount) fail(`existing U6 INR top-up amount conflicts; required=${requiredAmount}; existing=${topUp.amount}`);
    if (topUp.status === "PENDING") await admin.patch(`/api/v1/admin/financial/wallet-top-up-requests/${topUp.topUpReference}/decision`, { decision: "APPROVE" });
    topUp = (await WalletTopUpRequest.findById(topUp._id).lean() as unknown as TopUpRead | null);
    if (!topUp) throw new Error("G6 runtime safety check failed: U6 INR top-up disappeared after approval");
    let funding = await InternalTopUpFunding.findOne({ topUpRequestId: topUp._id }).lean();
    if (topUp.status === "APPROVED") await admin.post(`/api/v1/admin/financial/wallet-top-up-requests/${topUp.topUpReference}/start-processing`, { outcome: "SUCCESS" });
    funding = await InternalTopUpFunding.findOne({ topUpRequestId: topUp._id }).lean();
    if (!funding || funding.status !== "SUCCEEDED") throw new Error(`G6 runtime safety check failed: U6 INR provider funding is not SUCCEEDED for ${topUp.topUpReference}`);
    topUp = (await WalletTopUpRequest.findById(topUp._id).lean() as unknown as TopUpRead | null);
    if (!topUp) throw new Error("G6 runtime safety check failed: U6 INR top-up disappeared before accounting");
    if (topUp.status !== "COMPLETED") await admin.post(`/api/v1/admin/financial/wallet-top-up-requests/${topUp.topUpReference}/complete-accounting`, {});
    const [finalTopUp, finalWallet, ledger] = await Promise.all([WalletTopUpRequest.findById(topUp._id).lean(), Wallet.findOne({ userId: u6._id, currency: "INR" }).select("currency availableBalance reservedBalance lockedBalance currentBalance").lean(), LedgerEntry.find({ topUpRequestId: topUp._id, source: "INTERNAL_TOP_UP_FUNDING" }).select("direction account amount currency transactionId").lean()]);
    if (!finalTopUp || finalTopUp.status !== "COMPLETED" || !finalWallet || finalWallet.availableBalance < requiredAmount || ledger.length !== 1 || !ledger.some((entry) => entry.direction === "CREDIT" && entry.account === "CASH" && entry.amount === requiredAmount && entry.currency === "INR")) throw new Error("G6 runtime safety check failed: U6 INR top-up accounting or Wallet projection integrity check failed");
