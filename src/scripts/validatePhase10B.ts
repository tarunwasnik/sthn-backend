...
for (const file of required) expect(exists(file), `Missing Phase 10B file: ${file}`);
...
hasAll(pricing, ["CUSTOMER_PLATFORM_FEE_RATE_BPS = 500",
  ...
  "Number.isSafeInteger", "marketplacePricingService"],
"Shared pricing authority incomplete");
expect(!pricing.includes("0.05") && !pricing.includes("0.2"),
  "Pricing authority must use integer basis points only.");
for (const field of ["serviceAmount", "platformFeeAmount", "totalAmount",
  "currency"]) {
  const fieldBody = booking.slice(booking.indexOf(`${field}: {`));
  expect(fieldBody.slice(0, 240).includes("immutable: true"),
    `Booking pricing field is not immutable: ${field}`);
}
...
hasAll(reservation, ["booking.totalAmount", "LedgerAccount.WALLET_AVAILABLE",
  "LedgerAccount.WALLET_RESERVED", "walletProjectionService"],
"Wallet reservation total is incomplete");
hasAll(release, ["booking.totalAmount", "walletProjectionService"],
"Wallet release total is incomplete");
...
hasAll(accounts, ["PLATFORM_SERVICE_FEE_REVENUE"],
  "Platform fee Ledger account missing");
...
hasAll(runtime, ["customerTopUpAmount: 1_050", "reserved: 1_050",
  ...
  "snapshotMarketplaceCounts", "withdrawalInput.amount.amount, 800"],
...
expect(fs.existsSync(path.join(workspace,
  "docs/implementation/phase-10b-customer-platform-fee.md")),
"Phase 10B documentation is missing.");
