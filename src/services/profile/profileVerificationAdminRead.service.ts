...
export const getAdminProfileVerificationDetail = async (verificationReference: string): Promise<AdminProfileVerificationDetailDto> => {
  ...
  return {
    ...
    gatedVerification: inference?.gatedPolicyAnalysis ? { policy: inference.gatedPolicyAnalysis.policy, gate1: { outcome: inference.gatedPolicyAnalysis.gate1.outcome, usableCaptureCount: inference.gatedPolicyAnalysis.gate1.usableCaptureCount, weakestPeerMedian: inference.gatedPolicyAnalysis.gate1.weakestPeerMedian ?? null, threshold: inference.gatedPolicyAnalysis.gate1.threshold }, gate2: inference.gatedPolicyAnalysis.gate2 ?? null, gate3: inference.gatedPolicyAnalysis.gate3 ?? null } : null,
  };
};
