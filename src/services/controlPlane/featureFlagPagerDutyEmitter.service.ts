...
interface FeatureFlagPagerDutyAlert {
  flagKey: string;
  severity: string;
  count: number;
  threshold: number;
...
export const emitFeatureFlagPagerDutyAlert = async (
  alert: FeatureFlagPagerDutyAlert
) => {
  const routingKey =
    process.env.PAGERDUTY_FEATURE_FLAG_INTEGRATION_KEY;
