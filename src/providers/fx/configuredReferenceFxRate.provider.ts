...
import { loadFxRateConfiguration } from
...
export class ConfiguredReferenceFxRateProvider implements FxRateProvider {
  constructor(private readonly config = loadFxRateConfiguration()) {}
  ...
  async getReferenceRate(
    ...
    if (!this.config.requestEnabled || !this.config.providerName ||
      !this.config.baseUrl) {
      throw new FxRateSnapshotError("FX provider is not configured.",
    ...
    try {
      ...
      throw new FxRateSnapshotError("FX provider URL is invalid.",
        "FX_RATE_PROVIDER_NOT_CONFIGURED", 502, error);
    }
    url.searchParams.set("baseCurrency", input.baseCurrency);
    url.searchParams.set("quoteCurrency", input.quoteCurrency);
    if (input.effectiveDate) {
      url.searchParams.set("effectiveDate",
    ...
    try {
      ...
      try {
        ...
        throw new FxRateSnapshotError("FX provider response is malformed.",
          "FX_RATE_PROVIDER_INVALID_RESPONSE", 502, error);
      }
      if (payload.baseCurrency !== input.baseCurrency ||
        ...
        throw new FxRateSnapshotError("FX provider response is invalid.",
      ...
      if (Number.isNaN(effectiveDate.valueOf()) ||
        ...
        throw new FxRateSnapshotError("FX provider effective date is invalid.",
      ...
      return {
        ...
        providerPublishedAt,
        rawResponseFingerprint: crypto.createHash("sha256").update(raw).digest("hex"),
      };
    } catch (error) {
      if (error instanceof FxRateSnapshotError) throw error;
      if (error instanceof Error && error.name === "AbortError") {
        throw new FxRateSnapshotError("FX provider request timed out.",
          "FX_RATE_PROVIDER_TIMEOUT", 502, error);
      }
      throw new FxRateSnapshotError("FX provider is unavailable.",
