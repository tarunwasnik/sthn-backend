...
exports.deriveWalletConversionAccountingIdentity = void 0;
const node_crypto_1 = __importDefault(require("node:crypto"));
const hash = (value) => node_crypto_1.default.createHash("sha256").update(value).digest("hex");
const projectionReference = (operationKey) => `WPO-${hash(operationKey).slice(0, 16).toUpperCase()}`;
const deriveWalletConversionAccountingIdentity = (input) => {
    const accountingKey = `wallet-conversion-accounting:${input.conversionReference}:` +
        input.providerExecutionReference;
    const accountingReference = `WCA-${hash(accountingKey).slice(0, 20).toUpperCase()}`;
    const accountingTransactionReference = `WCAT-${hash(`${accountingKey}:transaction`).slice(0, 24).toUpperCase()}`;
...
exports.deriveWalletConversionAccountingIdentity = deriveWalletConversionAccountingIdentity;
