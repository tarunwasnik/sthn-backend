...
const identity = between(orchestrator, "  private accountingIdentity(", "  private projectionReference(");
...
for (const forbidden of [
    ...
    expect(!identity.toLowerCase().includes(forbidden.toLowerCase()), `Accounting identity contains unstable input ${forbidden}.`);
}
const fundingValidation = between(orchestrator, "  private validateFundingIdentity(", "  private validateLedgerIdentity(");
...
const ledgerValidation = between(orchestrator, "  private validateLedgerIdentity(", "  private validateProjectionIdentity(");
...
const ledgerEstablishment = between(orchestrator, "  private async establishOrReuseLedger(", "  private async establishOrReuseProjection(");
...
expect(!orchestrator.includes("ledgerEntryRepository.update") &&
    !orchestrator.includes("LedgerEntry.findOneAndUpdate"), "Accounting orchestrator contains a generic Ledger mutation.");
const projectionValidation = between(orchestrator, "  private validateProjectionIdentity(", "  private validateProcessingRequestLinks(");
...
const projectionEstablishment = between(orchestrator, "  private async establishOrReuseProjection(", "  private async validateCompletedAccountingReplay(");
...
const processingLinks = between(orchestrator, "  private validateProcessingRequestLinks(", "  private async establishOrReuseLedger(");
...
const completedReplay = between(orchestrator, "  private async validateCompletedAccountingReplay(", "  private validateCompletionWinner(");
...
const guardRecovery = between(orchestrator, "  private async completeProcessingOrRecover(", "  async complete(");
...
expect(occurrences(guardRecovery, "await complete()") === 2, "Guarded completion must execute initially and retry exactly once.");
expect(occurrences(guardRecovery, "findByReferenceForAccounting") === 2, "Guard recovery must reload after each guarded completion loss.");
for (const duplicateEffect of ["createCredit", "applyProjectionMutation"]) {
    expect(!guardRecovery.includes(duplicateEffect), `Guard recovery can duplicate a financial effect via ${duplicateEffect}.`);
}
const winnerValidation = between(orchestrator, "  private validateCompletionWinner(", "  private async completedWinnerOrThrow(");
...
const publicComplete = between(orchestrator, "  async complete(", "\n  }\n}\n\nexport const");
const completedBranch = publicComplete.indexOf("request.status === WalletTopUpRequestStatus.COMPLETED");
...
expect(completedBranch >= 0 && completedBranch < ledgerStage && completedBranch < projectionStage, "COMPLETED replay does not exit before accounting-write stages.");
...
expect(publicComplete.indexOf("findByPostingKey(identity.postingKey)") <
    publicComplete.indexOf("this.establishOrReuseLedger"), "Prior Ledger stage is not discovered before Ledger establishment.");
expect(publicComplete.indexOf("findByOperationKey(identity.operationKey)") <
    publicComplete.indexOf("this.establishOrReuseProjection"), "Prior projection stage is not discovered before projection establishment.");
expect(requestRepository.includes("status: WalletTopUpRequestStatus.PROCESSING") && requestRepository.includes("status: WalletTopUpRequestStatus.COMPLETED"), "Top-up accounting completion is not a guarded PROCESSING to COMPLETED transition.");
...
expect(!requestRepository.includes("updateStatus") &&
    !requestRepository.includes("genericUpdate"), "Generic top-up status mutation was introduced.");
expect(requestRepository.includes('.select("+providerFundingId +ledgerEntryId +walletProjectionOperationId")'), "Accounting lookup does not select hidden request links.");
expect(requestModel.includes("providerFundingId:") &&
    requestModel.includes("ledgerEntryId:") &&
    requestModel.includes("walletProjectionOperationId:"), "Top-up request durable accounting fields are missing.");
expect(projectionRepository.includes("findById") &&
    projectionRepository.includes('.select("+fingerprint")') &&
    projectionRepository.includes("findByOperationKey"), "Projection operation replay lookups or hidden fingerprint selection are missing.");
...
expect(ledgerService.includes("async createCredit(") &&
    projectionService.includes("async applyProjectionMutation("), "Required bounded Ledger or Wallet projection abstraction is missing.");
for (const code of [
...
expect(!/this\.error\(\s*["'`][^"'`]*(fingerprint|idempotency|index|database filter|_id)/i.test(orchestrator), "Client-facing accounting error exposes internal persistence details.");
for (const forbiddenBoundary of [
    ...
    expect(!orchestrator.includes(forbiddenBoundary), `Phase 7F crosses a frozen domain boundary via ${forbiddenBoundary}.`);
}
for (const forbiddenMechanism of [
    ...
    expect(!orchestrator.toLowerCase().includes(forbiddenMechanism.toLowerCase()), `Forbidden Phase 7F mechanism found: ${forbiddenMechanism}.`);
}
expect(!orchestrator.includes("/ 100") && !orchestrator.includes("* 100") &&
    !orchestrator.includes("Math.round") && !orchestrator.includes("parseFloat"), "Phase 7F converts minor-unit amounts.");
expect(adminRoutes.includes("complete-accounting"), "Phase 7F accounting route is missing.");
