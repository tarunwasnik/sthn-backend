...
const srcRoot = path_1.default.resolve(__dirname, "..");
const backendRoot = path_1.default.resolve(srcRoot, "..");
const workspaceRoot = path_1.default.resolve(backendRoot, "..");
const read = (...parts) => fs_1.default.readFileSync(path_1.default.join(srcRoot, ...parts), "utf8");
...
const packageJson = fs_1.default.readFileSync(path_1.default.join(backendRoot, "package.json"), "utf8");
const testDirectory = path_1.default.join(srcRoot, "tests", "financial", "phase10d");
const testSources = fs_1.default.readdirSync(testDirectory, { recursive: true })
    .filter((entry) => typeof entry === "string" && entry.endsWith(".ts"))
    .map((entry) => fs_1.default.readFileSync(path_1.default.join(testDirectory, entry), "utf8"))
    .join("\n");
...
expect(currencies.includes("SUPPORTED_CURRENCIES") &&
    !requestService.includes("const SUPPORTED_CURRENCIES"), "Top-up introduced a duplicate supported-currency authority.");
expect(walletModel.includes("walletSchema.index({ userId: 1, currency: 1 }, { unique: true })"), "Wallet ownership uniqueness is missing.");
expect(containsAll(walletCreation, ["currencyMetadataService.normalize",
    "findByUserAndCurrency", "createZeroBalance", "code === 11000"]), "Wallet creation is not supported-currency-bound and idempotent.");
expect(containsAll(requestService, ["walletCreationService.createWallet",
    ...
    "existingWallet._id.equals(existing.walletId)", "isValidMoney"]), "Selected-currency Wallet resolution or replay binding is missing.");
expect(containsAll(requestModel, ["walletId", "currency", "amount",
    "idempotencyKey", "requestFingerprint", "immutable: true"]), "Immutable top-up request identity is incomplete.");
expect(containsAll(fundingModel, ["topUpRequestId", "amount", "currency",
    "idempotencyKey", "requestFingerprint"]), "Provider funding identity is incomplete.");
expect(containsAll(fundingOrchestrator, ["linked.amount !== request.amount",
    "linked.currency !== request.currency", "currency: request.currency"]), "Provider replay is not request-currency-bound.");
expect(containsAll(accounting, ["funding.currency !== request.currency",
    ...
    "LedgerSource.INTERNAL_TOP_UP_FUNDING", "walletProjectionService"]), "Ledger or projection accounting is not bound to the selected currency Wallet.");
expect(containsAll(projection, ["normalizeWalletCurrency", "findByUserAndCurrency",
    "applyConditionalDelta", "operationKey", "fingerprint"]), "Wallet projection currency isolation or replay authority is missing.");
expect(!adminDecision.includes("exchangeRate") &&
    !containsAll(adminDecision, ["input.currency", "input.amount", "input.walletId"]), "Admin decision can alter immutable top-up intent.");
expect(containsAll(walletQuery, ["findAllByUser", "listWallets"]) &&
    containsAll(walletRoutes, ["/all", "listWallets", "/currencies", "listCurrencies"]), "Authenticated Wallet or currency listing is missing.");
expect(containsAll(walletController, ["req.user", "listWallets",
    "currencyMetadataService.listEnabled"]), "Wallet listing is not authenticated or metadata-backed.");
expect(containsAll(walletDto, ["currency", "available", "reserved", "locked",
    ...
    !walletDto.slice(walletDto.indexOf("WalletListItemResponseDto"))
        .includes("requestFingerprint"), "Wallet list DTO is unsafe or incomplete.");
expect(containsAll(currencyDto, ["code", "displayName", "symbol", "minorUnits",
    "walletEnabled", "topUpEnabled"]) &&
    !/exchangeRate|providerConfig|spread/i.test(currencyDto), "Currency metadata DTO is unsafe or includes FX data.");
expect(containsAll(testSources, ["Promise.allSettled", "ten USD Wallet",
    "identical USD accounting", "INR, USD, and EUR", "no lost update"]), "Required genuine concurrency tests are missing.");
expect(containsAll(testSources, ["preserve the funded currency end to end",
    ...
    "completed USD request cannot link to EUR projection"]), "Cross-currency full-flow and integrity tests are missing.");
...
expect(containsAll(testSources, ["request persistence failure",
    ...
    "aborted EUR projection", "completion-link interruption"]), "Staged rollback and recovery tests are missing.");
...
expect(testSources.includes("collection.indexes()") &&
    testSources.includes("index.key.currency === 1"), "MongoDB identity index verification is missing.");
expect(packageJson.includes('"validate:phase10d"') &&
    packageJson.includes('"test:phase10d"'), "Phase 10D package commands are missing.");
expect(fs_1.default.existsSync(path_1.default.join(workspaceRoot, "docs", "implementation", "phase-10d-direct-multi-currency-top-up.md")), "Phase 10D documentation is missing.");
...
expect(!/external.?provider/i.test(productionScope), "Phase 10D contains external provider integration.");
expect(!/booking|settlement|withdrawal/i.test([requestService, fundingOrchestrator, accounting, projection].join("\n")), "Direct top-up services crossed a frozen financial boundary.");
