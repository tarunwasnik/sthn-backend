...
const exists = (file) => fs_1.default.existsSync(path_1.default.join(root, file));
...
expect(database.includes('process.env.NODE_ENV, "test"') &&
    ...
    database.includes("disconnectPhase7HDatabase"), "MongoDB runtime isolation or cleanup guard is incomplete.");
...
expect(replay.includes("const immediate") && replay.includes("Admin endpoint") &&
    replay.includes("ledgerCount, 1") && replay.includes("projectionCount, 1"), "Completed replay runtime proof is incomplete.");
expect(concurrency.includes("Array.from({ length: 10 }") &&
    ...
    concurrency.includes("400") && concurrency.includes("3_900"), "Same-request or same-Wallet concurrency proof is incomplete.");
for (const marker of [
...
expect(reconciliation.includes("concurrent inspections") &&
    ...
    reconciliation.includes("concurrent requests"), "Reconciliation classification, deduplication, or retry proof is incomplete.");
expect(repair.includes("exact replay is idempotent") &&
    ...
    repair.includes("repair forbidden"), "Repair idempotency, race, or forbidden-repair proof is incomplete.");
expect(integrity.includes("completed corruption fails closed") &&
    ...
    integrity.includes("explicit session"), "Corruption, integrity, index, or session proof is incomplete.");
expect(booking.includes("Payment.countDocuments") &&
    ...
    booking.includes("audit persistence"), "Booking boundary, authorization, or audit proof is incomplete.");
...
const docs = path_1.default.join(workspace, "docs/implementation/phase-7h-runtime-financial-verification.md");
expect(fs_1.default.existsSync(docs), "Phase 7H runtime documentation is missing.");
