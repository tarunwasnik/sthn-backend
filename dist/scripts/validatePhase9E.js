...
for (const file of required)
    expect(exists(file), `Missing Phase 9E file: ${file}`);
...
hasAll(classification, ["HEALTHY_COMPLETED", "HEALTHY_FAILED",
    ...
    "MISSING_AUDIT", "MISSING_FINALIZATION_LINKS", "INTEGRITY_FAILURE", "UNKNOWN"], "Operational classifications incomplete");
...
hasAll(reconciliationModel, ["snapshot", "snapshotFingerprint", "retryCount",
    ...
    "status: 1, nextRetryAt: 1", "severity: 1, createdAt: -1"], "Reconciliation authority incomplete");
...
hasAll(repairModel, ["repairKey", "snapshotFingerprint", "repairedFields",
    "RESTORE_FINALIZATION_LINKS", "RESTORE_TERMINAL_AUDIT"], "Repair authority incomplete");
...
hasAll(retry, ["FINALIZATION_PENDING_SUCCESS", "FINALIZATION_PENDING_FAILURE",
    ...
    "creatorWithdrawalFinalizationService.finalize", "AFTER_PHASE9D_FINALIZATION"], "Retry authority incomplete");
expect(!retry.includes("ledgerService") && !retry.includes("walletProjectionService"), "Retry reimplements Phase 9D accounting.");
hasAll(repair, ["RESTORE_FINALIZATION_LINKS", "RESTORE_TERMINAL_AUDIT",
    "restoreFinalizationLinks", "validateReplay", "snapshotFingerprint"], "Bounded repair incomplete");
for (const forbidden of ["LedgerEntry.update", "Wallet.findOneAndUpdate",
    ...
    if (["amount:", "currency:"].includes(forbidden))
        continue;
...
hasAll(controller, ["req.user.id", "RESTORE_FINALIZATION_LINKS",
    "RESTORE_TERMINAL_AUDIT", "ACKNOWLEDGE", "RESOLVE"], "Thin authenticated controller incomplete");
hasAll(routes, ["router.use(protect, authorizeRoles(\"admin\"))",
    ...
    "/creator-withdrawal-reconciliations/:reconciliationReference/repair"], "Admin-only routes incomplete");
hasAll(error, ["WITHDRAWAL_NOT_FOUND", "RECONCILIATION_NOT_FOUND",
    ...
    "TRANSACTION_CONFLICT", "REPLAY_CONFLICT", "INTEGRITY_ERROR"], "Operational error contract incomplete");
hasAll(tests, ["healthy completion and failure", "provider initialized and processing",
    ...
    "interruptions roll back", "admin endpoint is protected"], "Runtime proof incomplete");
...
hasAll(phase9d, ["ledgerService.createDebit", "ledgerService.createCredit",
    "walletProjectionService.applyProjectionMutation"], "Phase 9D finalization authority was weakened");
expect(packageJson.includes('"validate:phase9e"') &&
    packageJson.includes('"test:phase9e"'), "Phase 9E package scripts missing.");
expect(node_fs_1.default.existsSync(node_path_1.default.join(workspace, "docs/implementation/phase-9e-withdrawal-operational-integrity.md")), "Phase 9E documentation missing.");
