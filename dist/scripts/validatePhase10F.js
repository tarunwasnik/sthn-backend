...
const quote = read("services", "financial", "walletConversionQuote.service.ts");
const service = read("services", "financial", "walletConversionRequest.service.ts");
...
const testRoot = path_1.default.join(src, "tests", "financial", "phase10f");
...
expect(status.includes('PENDING = "PENDING"') &&
    ...
    !/PROCESSING|CANCELLED/.test(status), "Conversion request foundation contains an invalid intermediate state.");
expect(all(repository, ["createPending", "findByReference",
    "findByUserAndIdempotencyKey", "listByUser", "findPendingByUser"]) &&
    !/genericUpdate|updateStatus|delete|setStatus/.test(repository), "Request repository does not preserve the narrow Phase 10F creation contract.");
expect(all(controller, ["req.user.id", "Idempotency-Key", "Object.keys",
    "sourceCurrency", "targetCurrency", "sourceAmount"]) &&
    !/req.body.userId|req.body.wallet|req.body.snapshot|req.body.rate/.test(controller), "Authenticated ownership or strict request-body authority is missing.");
expect(all(service, ["findByUserAndIdempotencyKey", "findByUserAndCurrency",
    "validateWallet", "availableBalance < amount", "targetWallet?._id"]), "Source validation or optional target-Wallet binding is missing.");
expect(all(quote, ["currencyMetadataService", "sourceCurrency === targetCurrency",
    ...
    !/parseFloat|Math\.round|Number\(.*rate/.test(quote), "Safe minor-unit target calculation is missing.");
...
expect(!/getReferenceRate|lookupOrRefresh|\.refresh\(/.test(service), "Phase 10F calls or refreshes an FX provider.");
expect(all(service, ["isValidIdempotencyKey", "requestFingerprint",
    "conversionKey", "IDEMPOTENCY_CONFLICT", "fxSnapshotReference"]), "Idempotency and snapshot-stable replay are incomplete.");
expect(all(routes, ["/conversion-requests", ":conversionReference", "protect"]), "Authenticated conversion routes are missing.");
expect(all(dto, ["conversionReference", "status", "sourceCurrency",
    ...
    !/userId|WalletId|idempotency|fingerprint|snapshotKey|actor|admin/.test(dto), "Safe request DTO is incomplete or leaks internal authority.");
expect(auditAction.includes("WALLET_CONVERSION_REQUEST_CREATED") &&
    all(audit, ["auditKey", "conversionReference", "fxSnapshotReference",
        "requestedAt"]), "Safe request audit is missing.");
...
expect(!/LedgerEntry|ledgerService|WalletProjection|walletProjectionService/.test(production), "Phase 10F implements accounting.");
expect(!/createWallet|applyConditionalDelta|applyProjectionMutation/.test(production), "Phase 10F mutates or auto-creates a Wallet.");
expect(!/AdminDecision|approvePending|rejectPending|approvedAt|rejectedAt/.test([quote, service, controller].join("\n")), "Phase 10F request creation implements Admin decision authority.");
expect(!/InternalProvider|ProviderExecution|ConversionExecution/.test([quote, service, controller].join("\n")), "Phase 10F request creation implements provider execution.");
...
expect(fs_1.default.existsSync(path_1.default.join(workspace, "docs", "implementation", "phase-10f-wallet-conversion-request-foundation.md")), "Phase 10F documentation is missing.");
