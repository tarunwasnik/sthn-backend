...
for (const file of required)
    expect(exists(file), `Missing Phase 8F file: ${file}`);
...
for (const service of [inspection, reconciliation, retry, repair]) {
    for (const forbidden of [
        ...
        expect(!service.includes(forbidden), `Forbidden Phase 8F dependency found: ${forbidden}`);
}
expect(!/Wallet\.(update|findOneAndUpdate|findByIdAndUpdate|create)/.test([inspection, reconciliation, retry, repair].join("\n")), "Phase 8F directly mutates a Wallet.");
hasAll(tests, [
...
expect(packageJson.includes('"validate:phase8f"') &&
    packageJson.includes('"test:phase8f"'), "Phase 8F package commands missing.");
expect(node_fs_1.default.existsSync(node_path_1.default.join(workspace, "docs/implementation/phase-8f-settlement-operational-integrity.md")), "Phase 8F documentation missing.");
console.log("Phase 8F settlement operational integrity static validation passed; MongoDB behavior requires test:phase8f.");
