...
hasAll(status, ['PENDING = "PENDING"', 'RESERVED = "RESERVED"'], "Withdrawal reservation statuses missing");
for (const forbidden of [
    ...
    expect(!service.includes(forbidden), `Phase 9A implements later withdrawal finalization: ${forbidden}`);
}
hasAll(model, [
    "withdrawalReference",
    "withdrawalKey",
...
hasAll(accounts, ['WITHDRAWAL_RESERVED = "WITHDRAWAL_RESERVED"'], "Withdrawal Ledger account missing");
hasAll(entryTypes, [
...
for (const forbidden of [
    ...
    expect(!requestController.includes(forbidden), `Phase 9A request entrypoint contains forbidden authority: ${forbidden}`);
}
for (const forbidden of [
    "InternalPayment",
    ...
    expect(!service.includes(forbidden), `Phase 9A service contains forbidden later-phase dependency: ${forbidden}`);
}
expect(!/Wallet\.(update|findOneAndUpdate|findByIdAndUpdate|create)/.test(service), "Phase 9A service directly mutates Wallet.");
hasAll(tests, [
...
expect(packageJson.includes('"validate:phase9a"') &&
    packageJson.includes('"test:phase9a"'), "Phase 9A package commands missing.");
expect(node_fs_1.default.existsSync(node_path_1.default.join(workspace, "docs/implementation/phase-9a-withdrawal-wallet-reservation.md")), "Phase 9A documentation missing.");
console.log("Phase 9A Creator withdrawal Wallet reservation static validation passed; MongoDB behavior requires test:phase9a.");
